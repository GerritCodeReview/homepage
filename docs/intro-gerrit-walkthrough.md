To understand how Gerrit works, let’s follow a change through its entire life
cycle. This example uses a Gerrit server configured as follows:
...
![Gerrit Code Review Screen](images/intro-quick-new-review.jpg)
...
In general, the **Code-Review** check requires an individual to look at the
code, while the **Verified** check is done by an automated build server, through
a mechanism such as the [Gerrit Trigger Jenkins
...
feedback. When she is satisfied with her review, Hannah clicks the **Publish
Comments** button. At this point, her vote and cover message become visible to
to all users.
