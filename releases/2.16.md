...
* Vast majority of permissions are defined on "real" branches. Having to define
a small subset of permissions on `refs/for/*` hinders readability and the
understanding of the applied ACLs.

* For people using `refs/publish/` to upload reviews, it is even more more
confusing applying the push rules on `refs/for/` to allow creation of changes.
...
* `Add Patch Set` will work the same way, but should specified on
`refs/heads/*` (just like the new `Create Review`-permission).

* `Push Merges` will work the same way, but should specified on `refs/heads/*`
isntead of `refs/for/*`.
