...
* Allow to select and copy text in inline comment boxes.
  ...
  Since Chrome v62 it is not possible to select/copy text from an element
  that has the `user-select` property set to `initial`.

  The property is now set to `text` for all browsers.

* Upgrade GWT to version 2.8.2.

  This version includes a fix for [GWT issue 9542](https://github.com/gwtproject/gwt/issues/9542)
  (incorrect getAbsoluteLeft/Top values in Chrome 61 and later)
  and various other fixes since 2.8.0.

* Fix status transition when force pushing an abandoned change.

  If a change was abandoned, but then force pushed to the branch, its
  status was not changed from "Abanconed" to "Merged".
