...
Version 2.14.4 includes a change to the JdbcAccountPatchReviewStore primary
key.

Sites that have already upgraded from an earlier version to 2.13, or to a 2.14.x
version before 2.14.4, need to manually drop and recreate the primary key as
follows:
...
This step is not necessary when upgrading directly to 2.14.4 from a version
earlier than 2.13.
...
### 2.14.4 (in development) {#2.14.4}

* Fix performance issue when clearing reviewed flag for a patch set.
+
Change the order of fields in the primary key of the JdbcAccountPatchReviewStore
table, to allow usage of implicit indices when querying rows.
+
...
+
The list of supported options was converted to lower case, but the
input prompt did not do the same for the default value.
