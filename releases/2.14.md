...
* Keep old timestamps during data migration.
  ...
  During data migration to replace the "Submitted" status with "New", the
  "created on" field was updated to the time of the migration.

* Don't create GC log file when GC is not configured.

* Specify new tip of the branch when updating submodules.

  In Gerrit2.12, whenever a submodule was updated the corresponding commit
