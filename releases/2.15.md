...
Download: **[2.15-rc3](https://www.gerritcodereview.com/download/gerrit-2.15-rc3.war)**
...
The only account data not stored in NoteDb proper is the per-user/change/file
"reviewed" flags, which were moved to an [extension
point](https://gerrit-documentation.storage.googleapis.com/Documentation/2.15/dev-plugins.html#account-patch-review-store).
The default implementation uses a
[separate SQL database](https://gerrit-documentation.storage.googleapis.com/Documentation/2.15/config-gerrit.html#accountPatchReviewDb)
from ReviewDb. During the migration, data is copied to a local H2 database, but
it can be
[moved](https://gerrit-documentation.storage.googleapis.com/Documentation/2.15/pgm-MigrateAccountPatchReviewDb.html)
to another database at any time.
