...
* Replaces MySQL connector with MariaDB connector
...
### MySQL connector replaced with MariaDB connector

The MySQL connector has been replaced with MariaDB connector.

The reason why is because the mysql update 6.x is broken. It dosen't mention anything
that would concern us api rise. The MariaDB connector works. They say it also works with mysql too.

You will need to remove the mysql connector from the lib folder lib/
...
If for some reason you need to use the mysql connector you can do the following

(Note: That mysql connector 6.x+ will not work, you can download from
...
* [Work-in-progress
  changes](https://gerrit-documentation.storage.googleapis.com/Documentation/2.15/user-upload.html#wip)
