...
Edwin Kempin (Google) is one of the historical maintainers of the Gerrit Code
Review project. He one of the Community Managers that is working hard to reinforce
...
can be improved in the whole project, giving completely freedom to anyone to
paticipate either on-site and on-line, using a digital ideas board.

The full report has been now published and is available on the
...
source components.
The CI/CD pipeline of Volvo Cars also includes some key components, like Zuul
...
The maintainers of the Zuul project has explicitly shown interest in
...
Monty Taylor (RedHat) has introduced Zuul, an OpenSource project created for
...
Zuul jobs definition does not differ much from the Jenkins-Job-Builder YAML
but relies on Notepool and Ansible for job scheduling and execution.

Monty and James have already started work to implement
...
James E. Blair (RedHat) has impressed the audience with its revolutionary TTY-based
...
experience by only using your keys.

During the daily work of the software developers, the keyboard is the central part
...
flow. Gertty comes to the rescue and gives to everyone that feel more productive with
...
This was also one of the main "goals" of the Gerrit project foundeer, Swan Pearce: using
...
and the meta-data is stored in a SqlLite DBMS on the local computer. However, James was
open to the idea to get rid in the future of the local DB and fully support the NoteDb
format frmo Gertty, which it would be great as it would be exactly the way that Shawn
initially design the Gerrit product.

James also made, during the Hackathon, important fixes to Gertty to make sure that is
...
underway on the current master of the Gerrit Code Review project.
...
- Before joining Softagram, Michael has never used Gerrit before. However, he managed
...
The integration will further grow soon and, as Gerrit Community, we are
fully committed to helping Michael and all the other new contributors to get on with
Gerrit development and joining for making it better and easier to use.
...
4. European summits should be in cheaper countries than Scandinavia; it can be hard to
   ...
   potentially be a cheaper destination for the next Summit in 2020, assuming to be
   able to find a company that is willing to host the event.
