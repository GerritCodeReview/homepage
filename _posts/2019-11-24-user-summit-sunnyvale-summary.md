...
- Consistency on demand and update indeces, which allows Git servers on multiple
...
- Sideband-all, which means that at any point in the communication the client
  and server can pass parallel information to the normal Git protocol client/server
  communication. That allows new use-cases like the Packfile off-loading, which
  is a new capability that would communicate a series of mirrors where the packfiles
...
- Local reftables, presented by Han-Wen, are an innovative storage format that
  ...
  the access time on the filesystem.
...
Gerrit started as a Maven build project (until v2.17) and then later moved to Buck
...
integration-tests in Gerrit, using the TestContainers library, which allowed to automatically
...
reported by Luca Milanesio (GerritForge) and later fixed by adjusting the way packfiles cache
...
regression observed.
The reason why JGit had historically the 2.5s resolution FAT filesystem of 2s (the extra .5s is a safety margin),
which can still be found in some Windows systems running Eclipse.
