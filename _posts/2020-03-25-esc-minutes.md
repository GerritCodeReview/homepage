...
* [Instance ID / name propogation in events](https://gerrit-review.googlesource.com/c/homepage/+/257972)
...
suggested using some kind of annotation. Everyone agreed that this is probably
...
Alice will propose documentation updates to formalize this.
...
We concludd that we will not pursue this now. We may revisit it later.
...
Alice pointed out that it's not only the UI and error messages that need
to be localized. Gerrit also stores hard-coded English strings in NoteDb
...
to keep up with the latest versions. Alice approved and subitted the guice
...
similar), but this would need to be coordinated by attendees; it's not
something that Googlers can set up.
