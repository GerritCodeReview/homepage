...
**Participants**: Daniele Sassoli [DS], David Ostrovski [DO], Ivan Frade [IF], Matthias Sohn [MS], Nasser Grainawi [NG], Terry Parker [TP], Luca Milanesio [LM]

**Next meeting**: May 6th, 2026

Here are the revised meeting minutes, formatted with a strict 100-column word wrap.

***

## Gerrit Code Review: Meeting Minutes & Action Points

### Executive Summary

During our latest Gerrit Code Review sync, we addressed critical operational, security, and
technical initiatives, alongside a leadership transition as [TP] hands management of the Google
...
targeted load tests and automated invariants—to maintain project stability and quality as we
evolve.
...
would allow them to remove SPAM while ensuring all compliance records are strictly maintained.

### 4. Replication and Infrastructure Issues
The team addressed recent failures in the replication mechanism on
`gerrit-review.googlesource.com`. Specifically, the "JGit" `servlet-4` branch from the
`eclipse.gerrithub.io` repository, as well as several security fix branches, were failing to land
correctly on Gerrit @Google.
...
25 is available internally and that Google has no objections to the project moving forward with
this upgrade. 
...
Java 21.
* Starting from **Gerrit v3.16**, Java 25 will become mandatory, and support for Java 21 will be
formally dropped.
...
2. **Content Moderation:**
    * **Project Leadership:** Initiate discussions with internal trust teams at Google to
    ...
    comments) can be legally and safely extended to designated community managers outside of
    Google.
