...
To prevent the two thread, even on different nodes, from running the same task, introduce a URI
based filesystem "lock" that can only be acquired atomically by one thread, and only run tasks on
the thread which acquires the lock. URI based locking will be used instead of task level locking to
...
tasks for that URI should be run be after moving the task file from the "waiting" directory into the
...
assume the URI is already being replicated by another node.
...
#### Task deduplication across the cluster

Task deduplication will be impacted by the length of the distribution interval, by the length of
...
opportunities there are for deduplication to occur.
...
are available. So an individual replicate all will scale better on the master nodes with more nodes,
...
### 1) Prevent one master from overwriting an already existing task in the store.

Write new task first to a tempfile in another directory, then move the tempfile into the 'waiting'
directory. To avoid resetting the timestamp, do not overwrite already existing files in the
"waiting" directory.

### 2) Prevent Concurrent replication to the same URI

Introduce a URI based filesystem "lock" that can only be acquired atomically by one thread, and only
run tasks on the thread which acquires the lock.  Before running a task, acquire the URI lock by
creating a directory with a uriKey (the sha1 of the URI). If the directory can be created, then run
all the tasks for that URI after moving the task files from "waiting" directory into the "running"
lock directory of the URI.

On startup after continue to moving running tasks back to the waiting directory, delete the URI lock
directory for each previously running task.

Modify the persistent task listing code to iterate over the contents of each running subdirectory.

### 3) Periodically add all persisted tasks to the replication queue

Create a Distribution Lifecycle Listener that runs periodically with a configurable interval.
On each interval, read each waiting task and its timestamp and schedule the task to run one
"replication" delay after the timestamp of the task. Use the timestamp of the oldest persisted task
file for the same URI.

### 4) Add random delays to scheduling tasks on each node

When adding tasks from the persistent store into a nodes queue, add a small random delay to the
scheduled start time of each task where the delay upper bound is equal to the distribution interval.

## <a id="time-estimation"> Time Estimation

1-4 can each be prototyped in around 1-3 hours each, and likely can be completed and uploaded and
