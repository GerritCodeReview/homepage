...
gitReferenceUpdatedListener.Event. The replication plugin listens to these events to determine how
to replicate the update. First it splits the project/ref based event (update to a ref in a specific
...
For example:
 ...
 Event Fired by Gerrit for -> (project="All-Porjects", ref="ref/meta/config")

 Replication plugin config (similar to example in use-case doc) splits the event into 5 git pushes:
...
After the specific URI/ref based tasks are created, these are persisted to the filesystem,
each in a json file named after the SHA1 of the json under
...
After persisting a task, it is scheduled to be run after a configurable replication delay. The
scheduled task is put into a thread pool managed by Gerrit with a java ScheduledThreadPoolExecutor.
The specific thread pool depends on the replication configuration. Since there are a limited amount
of threads, the tasks may wait longer than the replication delay before executing. On scheduling the
plugin attempts to consolidate the push with any existing pushes for the the same URI which may
still be waiting to run.
...
pushed with a single git push, and the persisted tasks for a URI are moved to the running directory.
...
If this push fails, it may either be aborted, or a retry may be rescheduled for a later time (moving
the tasks back to the waiting directory). On success, the persisted json file will be deleted from
the filesystem. Finally, on system startup, all the existing persisted tasks will be replayed with
...
To avoid resetting timestamps, do not overwrite already existing files in the "waiting" directory.
...
Example:
  ...
  Lock dir: <site_dir>/data/replication/ref-updates/running/23baf46.../   (SHA1 of URI)
...
#### Task consolidation the cluster
