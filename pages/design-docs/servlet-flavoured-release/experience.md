...
* `bazel-bin/release.war` — the EE8 (`javax.servlet`) flavour, the **unchanged
  default**.
* `bazel-bin/release-ee11.war` — the EE11-labelled (`jakarta.servlet`) flavour.
...
gains its EE11-labelled flavour by adding a sibling target that reuses the
...
* wrap the jar in the EE11-labelled build configuration, so it self-selects the
...
The EE11-labelled jar is **generated** from the canonical EE8 source — no fork,
...
1. **`gitiles`** — core plugin (servlet-coupled; its EE11 flavour consumes the
...
3. **`javamelody`** — a custom (non-core) plugin classified as
   **servlet/filter-coupled**, and the first to use the
   ...
   `gerrit-plugin-api-ee11` are the remaining wiring.)
...
The EE11-only targets — the jakarta servlet libraries (`httpd-ee11`,
`oauth-ee11`, `openid-ee11`, `init-ee11`, `jetty-ee11`) and the `*-ee11` test
targets — compile *only* in the EE11 configuration, because their jakarta
...
are marked **`target_compatible_with` the EE11 flavour**: in the default flavour
Bazel reports them *incompatible* and **skips** them (it does not fail), and
under the flavour flag they become compatible and run.

```sh
# EE8 (default): the EE11-only targets are skipped automatically -- stays green.
bazelisk test //...

# EE11: run the jakarta test targets under the flavour flag.
bazelisk test --@com_googlesource_gerrit_bazlets//flags:flavour=ee11 \
  //javatests/com/google/gerrit/httpd:httpd_tests-ee11 \
  //javatests/com/google/gerrit/util/http:http_tests-ee11
```

Because the guard is *conditional on the flavour*, each target shows up exactly
when its flavour is active: skipped in the default pass, run in the EE11 pass —
so neither pass has to enumerate or exclude targets by hand. (The `-ee11`
*build/publish* targets — `release-ee11`, the `-ee11` plugin jars and APIs —
self-transition and need no flag at all; only the EE11 *test* targets take the
flag, because a test target cannot self-transition.)

So CI runs the suite twice: one flagless EE8 wildcard over the whole tree (EE11
targets auto-skipped), and a focused EE11 pass over the jakarta test targets.
