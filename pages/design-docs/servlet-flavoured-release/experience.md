...
3. **`javamelody`** — a custom (non-core) plugin classified as
   **servlet/filter-coupled**, and the first to use the
   ...
   against the published plugin API). This makes javamelody the intended
   end-to-end proof of the publishing story: a real servlet-coupled custom
   plugin building its jakarta flavour standalone against the released
   jakarta API — which, since publication lands with the default flip, is
   the unsuffixed `gerrit-plugin-api` (no `-ee11` Maven name is ever
   minted). Its endgame is prepared too: with the flavour machinery
   retired, javamelody converges to a single plain jakarta plugin against
   `javamelody-core` 2.x — the same convergence the core plugins make.
