...
This page describes the machinery of the **two-flavour era**. It is retired
at the end of the lifecycle
([`drop-flavour-machinery`](https://gerrit-review.googlesource.com/q/topic:drop-flavour-machinery));
this page then remains as its map — both for understanding the transition's
history and for resurrecting the machinery from git history if a future
servlet generation ever needs a coexistence window again.
...
| `flavoured_library` / `flavoured_tests` | **one-call mechanical twins** — generate the whole family (canonical leaf + namespace transform + guarded generated leaf + `flavoured_twin_alias`) from a single call; the `canonical` attribute names which flavour the sources are written in (jakarta-canonical since the source convergence, generating the ee8 twin via `to_javax`); `flavours` defaults to `KNOWN_FLAVOURS`, so a mechanical flavour needs no call-site edit once the transform produces its sources |
...
* **`select()` / `flavoured_alias` / `flavoured_twin_alias`** — for **twin
  ...
  differ* (`httpd-ee11` and the transform-generated `httpd-ee8`). A toolchain
  ...
  `:httpd-ee11` from the canonical jakarta sources, `:httpd-ee8` from the
  `to_javax` transform-generated sources — sharing FQDNs, so only one is ever on
  a classpath. The convention wrapper
...
*Superseded by the lifecycle's endgame decision: because
`jakarta.servlet` is version-stable, the next servlet generation (EE12) is
expected to arrive as an ordinary dependency bump, and the machinery below
is retired rather than kept waiting for it (see the
[Conclusion](/design-docs/servlet-flavoured-release-conclusion.html)). This
section remains as the record of the machinery's extensibility — and as the
recipe, should a future generation ever need a coexistence window after
all.*

The two-flavour rollout supports two build values: the EE8 default and
`ee11`, the jakarta servlet tier (Servlet 6.1 / Jetty EE11). The naming is
deliberately more general than that pair because the same shape could carry
later servlet generations, such as `ee12` (Jetty 13 / Servlet 7). The path
would always be the same — add the value to `KNOWN_FLAVOURS`, then add the
matching `config_setting` in bazlets and the concrete servlet-flavour
toolchain registration in Gerrit for the new bundle.
...
* **Mechanical twins gain the new flavour with no call-site change.**
  ...
  *provided the transform already produces that flavour's sources* (the
  canonical sources are jakarta and `jakarta.servlet` is version-stable across
  EE11/EE12, so a jakarta-family flavour is served as-is; a flavour needing a
  different source mapping extends the transform first, as the `to_javax`
  direction does for EE8).
...
are *not* a pure transform (e.g. `pgm/http/jetty`, whose `ee8/` overlay package
holds the classes with no mechanical javax form — `HiddenErrorHandler`,
`SameSiteFilter` — so its `flavour` and `flavoured_twin_alias` list stay
explicit), the concrete build outputs (`release-ee11`, a published `-ee11`
artifact), and plugin source-transform and manifest-marker logic.
