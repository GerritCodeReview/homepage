...
2.0.0 — while Gerrit core and the bulk of the plugin ecosystem are still on
`javax.servlet` (EE8). The starting point is therefore already heterogeneous —
not by design, but because the libraries advanced first — so consuming current
JGit forces a servlet-namespace decision. The flavour model does not create this
split; it manages it (and, per the lifecycle, converges it back to one).
...
* Others are ready to begin the `jakarta.servlet` transition now and should not
  be blocked waiting for the slower adopters. The current opt-in build is still
  labelled `ee10`, but its concrete servlet tier is Servlet API 6.1.0 with the
  corresponding Jetty EE11 environment.
