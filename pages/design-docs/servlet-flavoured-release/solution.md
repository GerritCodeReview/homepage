...
| `release-ee10.war` (opt-in) | Servlet API 6.1.0, `jakarta.servlet`, Jetty 12 (jakarta), Guice 7.0 | Sites whose plugin set is jakarta-ready. |

Both flavours are intended to be published to Maven Central side by side under
the same group and version, distinguished by the current `-ee10` artifactId
suffix — the WAR as `gerrit-war` / `gerrit-war-ee10`, and (so that plugins can
be built **standalone** against the matching API) the plugin API as
`gerrit-plugin-api` / `gerrit-plugin-api-ee10`. See
...
the EE10 flavour out to operators, are the remaining steps.
...
release-ee10.war (EE10-labelled / jakarta.servlet)
...
* **Forward-bridge** — Gerrit core is still `javax.servlet`-canonical, so it
  ...
  boundary and the in-tree plugin builds — for the `release-ee10.war` artifact.
