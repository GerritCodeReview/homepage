...
with the EE11 flavour **generated** from the canonical EE8 source by the shared
transform. The change series adds the matching plugin-API flavour, and the shared
`gerrit_plugin(flavour = "ee11", ...)` build macro does the rest in one line —
it runs the transform on the plugin's sources, injects the `Gerrit-Flavour: ee11`
manifest entry, compiles against the jakarta plugin API, and wraps the target so
it self-selects the EE11 tiers (detailed in
...
* **Servlet-coupled** — ship one artifact per flavour; generate the EE11 one
  with the one-line `gerrit_plugin(flavour = "ee11", ...)`.
