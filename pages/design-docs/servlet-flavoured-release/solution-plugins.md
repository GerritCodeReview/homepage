...
Plugins that touch servlet APIs publish one artifact per flavour: the EE11 jar is
marked with `Gerrit-Flavour: ee11`, while the EE8 jar may either be explicitly
marked `Gerrit-Flavour: ee8` or remain unmarked for compatibility with existing
plugins — though a plugin that ships an EE11 twin declares `ee8` explicitly:
the marker is loader-equivalent to the unmarked jar, and the attribute
additionally guards the javax side to the ee8 build configuration, which is
what keeps the per-plugin CI wildcard green under the EE11 flag (the
reference plugins all do this; see the migration path below). Audited
servlet-neutral plugins declare `Gerrit-Flavour: any` and ship a
single artifact.
...
* **Servlet/filter-coupled** — imports `javax.servlet`/`jakarta.servlet`,
  ...
  (`ee8` and `ee11`).
* **Unknown** — not yet audited: treated as EE11-incompatible until classified.
...
or `Gerrit-Flavour: ee8`) and `javamelody-ee11` (`Gerrit-Flavour: ee11`).
`javamelody-ee11` is now built as the first **custom** dual-flavour Gerrit
...
3. **Separately built per-flavour** — when generation cannot work because a
   ...
   So the Javamelody plugin's flavour split must follow it: the default
   `javamelody` (EE8) builds against the 1.x line and `javamelody-ee11`
   against the 2.x line.
...
with the other flavour **generated** from the canonical source by the shared
transform — in either direction. A still-`javax` plugin adds
`gerrit_plugin(flavour = "ee11", ...)` and the macro generates the jakarta
flavour; a plugin that has migrated its sources declares
`gerrit_plugin(canonical = "jakarta", flavour = "ee8", ...)` and the macro
generates the javax flavour instead (the backward bridge — how the migrated
core plugins `gitiles` and `plugin-manager` build today, migrated under the
[`jakarta-canonical`](https://gerrit-review.googlesource.com/q/topic:jakarta-canonical)
topic). Either way the macro
pair does the rest in two lines per flavour — the jar and its test twin:
`gerrit_plugin` runs the transform on the plugin's sources, injects the
matching `Gerrit-Flavour` manifest entry, compiles against the matching
plugin API, and wraps the target so it self-selects the flavour's tiers;
`gerrit_plugin_tests` with the same `flavour`/`canonical` attributes runs
the canonical test sources — transformed the same way — against that
flavour's plugin library, guarded to its configuration (detailed in
...
**Short recommendation: classify your plugin first, then mark it.**
...
* **Servlet-coupled** — ship one artifact per flavour, each with its test
  twin; generate the non-canonical side with
  `gerrit_plugin(flavour = "ee11", ...)` +
  `gerrit_plugin_tests(flavour = "ee11", ...)` (javax-canonical sources) or
  `gerrit_plugin(canonical = "jakarta", flavour = "ee8", ...)` and its
  test twin (migrated sources).
* **Do nothing** — an existing EE8-only plugin keeps working on the default
  ...
  EE11 until you publish an `ee11` or `any` build.
...
4. **Build** the EE11 flavour against the matching plugin API added by the change
   series, the Guice tier, and any split library release. For a clean namespace
   swap this is `gerrit_plugin(flavour = "ee11", ...)` plus its
   `gerrit_plugin_tests` twin.
5. **Test** each artifact on its matching WAR — with the build line's test
   twin, not by hand: `gerrit_plugin_tests(flavour = "ee11", ...)` executes
   the canonical test sources (transformed like the jar's) against the
   flavoured plugin library, guarded to its configuration, so the
   per-plugin wildcard is green with and without the flavour flag at every
   dual-flavour stage — each pass runs its own flavour's tests and skips
   the other. The obligation ends at EE8 retirement: the `ee8` flag value
   retires with the flavour, and only the jakarta suite and its single
   pass remain. A flavoured jar that is
   byte-audited but never test-executed is
   only half migrated. Publish `Gerrit-Flavour: ee11` only after its tests
   have run on the EE11 side; publish `Gerrit-Flavour: any` only after the
   audit confirms the jar has no servlet namespace dependency.
...
flavour from the bundled servlet-API namespace (`jakarta.servlet` → EE11,
otherwise EE8), and `JarPluginProvider` validates each JAR's marker as it
loads. An `ee8` or `ee11` marker loads only on the matching flavour; `any`
...
on EE11, where a stray `javax.servlet` plugin is unsafe. The marker itself is
...
The marker contract is itself transitional: it exists for the javax→jakarta
namespace chasm, and is retired one release train after the EE8 flavour is
removed (see the
[`drop-flavour-markers`](https://gerrit-review.googlesource.com/q/topic:drop-flavour-markers)
topic and the lifecycle in the
[Conclusion](/design-docs/servlet-flavoured-release-conclusion.html)), at
which point the loader ignores markers and returns to its pre-marker form.
