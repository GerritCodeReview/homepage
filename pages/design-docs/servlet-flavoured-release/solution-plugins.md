...
does the rest in one line: it runs the transform on the plugin's sources,
injects the matching `Gerrit-Flavour` manifest entry, compiles against the
matching plugin API, and wraps the target so it self-selects the flavour's
tiers (detailed in
...
* **Servlet-coupled** — ship one artifact per flavour; generate the
  non-canonical one with the one-line `gerrit_plugin(flavour = "ee11", ...)`
  (javax-canonical sources) or
  `gerrit_plugin(canonical = "jakarta", flavour = "ee8", ...)` (migrated
  sources).
...
4. **Build** the EE11 flavour against the matching plugin API added by the change
   ...
   swap this is the one-line `gerrit_plugin(flavour = "ee11", ...)`.
5. **Test** each artifact on its matching WAR. Publish `Gerrit-Flavour: ee11`
   only after it has run on an EE11 runtime; publish `Gerrit-Flavour: any` only
