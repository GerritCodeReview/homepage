...
Primary use case: Allow plugging global ref-db implementation to the multi-site plugin

AS A Gerrit administrator
GIVEN a multi-master/multi-site setup of Gerrit using the multi-site plugin
WHEN I want to plug another implementation of global ref-db
THEN I should have a consistent interface in Gerrit
SO THAT I can develop (or use an off-the-shelf) another implementation and plug it without changing any lines of code in Gerrit or the multi-site plugin.

Secondary use cases: Reuse the global ref-db with other plugins

AS A Gerrit developer
GIVEN a multi-master HA setup of Gerrit (active/active)
WHEN I want to allow all masters to write to the same refs at the same time
THEN I can rely on a global ref-db interface in Gerrit
SO THAT I don't have to "reinvent the wheel" in the HA plugin
