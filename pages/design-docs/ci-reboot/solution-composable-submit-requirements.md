...
Allow composing Submit Predicates to Submit Requirements using a light-weight,
...
With this project, we add a layer on top of the existing `SubmitRules`
interface.
The existing interface is renamed to `SubmitPredicate`. The new layer is called
`SubmitRequirement` and references `SubmitPredicates` in different queries.
Plugins can continue to contribute `SubmitPredicates`.
...
be submittable. SRs have two queries that work in the same way as change queries
do. One that defines applicability and one that defines a blocking condition.
The queries are built of Submit Predicates or Change Index Predicates that can
be composed together using AND, OR and NOT. We do not support nested predicates
or variables.
...
message SubmitRequirement {
  ...
  string blocking_condition = 4;   // Query, that if it evaluates to true will 
...
|Application Condition|`NOT commit_author(“service-account@google.com”) AND commit_filepath_contains(“/api/.*”)`                                             |
|Blocking Condition   |`label(“API Review”, MAX_WITH_BLOCK)`                                                                                                 |
|Override Condition   |`label(“Build-Cop-Override”, MAX_WITH_BLOCK)`                                                                                         |
|Can Override         |`is:false`                                                                                                                            |
...
The Gerrit@Google team will work on this in Q4/2020 and Q1/2021.