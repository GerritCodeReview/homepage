...
*Being in the attention set and being assigned is synonymous in this doc. "Owner" in this doc
...
Email notifications to reviewers will only be sent when they are added added to the attention set.
Emails about new patch sets will be not be sent to reviewers anymore.

## <a id="defaults">Defaults

In order to not create an extra burden for the user, we propose to apply useful default
modifications to the attention set when standard review events happen. Each of the following
defaults can be overridden when executing the action in the user interface:
 
*   When reviewers are added they are added to the attention set.
*   When a change is merged all users are removed from the attention set.
*   Publishing a change message (with or without comments or votes) will remove the publishing user
    from the attention set.
    *   When a *reviewer* publishes add the owner to the attention set.
    *   When the *owner* (or committer or uploader) publishes add all reviewers to the attention
        set.
