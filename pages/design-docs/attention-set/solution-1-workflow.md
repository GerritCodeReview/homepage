...
and obligations are the same for all users on all changes on all hosts. So as an owner for exmaple
...
We directly address four things that we perceive as shortcomings of the current "Assignee" feature
as were listed in [Use Cases](use-cases.md):
...
*   **Only one user can be assigned:** We allow multiple users to be assigned at the same time.
    (Note that for expressing urgency, which is what Assignee was designed for, this is actually a
...
We propose to keep the "Assignee" feature around, but new releases would it have turned off by
...
## <a id="snoozing">Deferred Enhancement: Snoozing
