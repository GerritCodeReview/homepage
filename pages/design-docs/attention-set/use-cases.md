...
*   The change page must have a clear indication at the top (e.g. in the change metadata) from whom
    action is required and which action is expected from which reviewer. This must be presented in
    the same way for all users looking at that page.
*   Both on the dashboard and the change page there must be a discoverable UI element for changing
    the state of whose action is required.
*   Providing a way to specify which action is expected from which reviewer.
