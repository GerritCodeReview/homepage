...
* Creation of accounts with usernames only different in capitalization will be
  prohibited by checking for existing usernames case insensitively.
* A migration path will be provided, including tooling and documentation for how
  to deal with duplicate usernames and schema migrations.
* Usernames will be stored case preserving.

## <a id="detailed-design"> Detailed Design

* A tool to detect duplicate accounts from NoteDB data. Already implemented
  in [change 300308](https://gerrit-review.googlesource.com/c/gerrit/+/300308)
* Document how duplicate accounts can be dealt with without causing issues due
...
* The data migration will require time during the update and might not be possible
  to do online.
* Existing duplicate accounts will have to have their externalIds deleted or
  renamed manually, which will disrupt clients using these accounts and potentially
  create quite some work for Gerrit administrators to decide which of the accounts
  to keep.
