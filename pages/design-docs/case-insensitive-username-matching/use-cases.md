...
* external IDs: Gerrit uses (generic) user names in different externalId schemes
  ...
  The schemes relevant for authentication are: `gerrit`, `username`, `external`,
...
* Prohibit usernames only different in capitalization that make it harder for
  users to identify such a user.

## <a id="acceptance-criteria"> Acceptance Criteria

* An administrator should be able to prohibit the creation of accounts with
  usernames that are only different in capitalization. If case insensitive
  username matching is configured, this should be implied.
...
lowercase, since this would break the user branches feature, which uses the
username in the branch name.
