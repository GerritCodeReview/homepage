...
* A change can either have a Jujutsu change ID or a Gerrit Change-Id, but not
  both. If an uploaded commit has both change ID's, the Jujutsu change ID from
  the commit header takes precedence.
...
* Changes that have a Gerrit Change-Id cannot be reworked/rebased/cherry-picked
  locally by users that use Jujutsu.
