---
title: "Design Doc - Code Search for Gerrit - Solution: Zoekt-based Search Platform"
permalink: design-docs/code-search-solution.html
hide_sidebar: true
hide_navtoggle: true
toc: false
...
# Solution: Zoekt-based Search Platform
...
![Code search high-level architecture](/images/code-search-architecture.png)
...
* **Search plugin** — the only user-facing entry point and trust boundary.
  ...
  REST API. Each result links to the repository browser configured for the
  deployment: Gitiles is the primary integration, while a configured
  `config-gitweb` URL can be used when Gitweb is the site's browser instead.
  If neither browser integration is available, search still works but the
  result cannot provide a file-view link. Also exposes, for the same
  allowed scope, which of the user's readable projects and branches are
  currently indexed.
...
* **Search gateway** — the entry point for scoped queries and coverage
  requests. It accepts requests only from authenticated, deployment-
  authorized service callers (for example, the Gerrit plugin using
  mTLS/service identity or an equivalent short-lived service credential),
  and rejects unauthenticated or unregistered callers before processing the
  request. This caller authentication is separate from end-user
  authorization: the gateway does not trust the caller merely because it
  supplies a scope. It translates the caller-supplied `{server, project, refs}`
...
**Use OpenGrok as the search backend**

OpenGrok indexes and authorizes at project granularity, not per branch or
ref, so it has no native way to represent Gerrit's ref-level access model.
Giving OpenGrok branch-awareness means synthesizing it externally — for
example a separate working-tree checkout and index per branch, or a
project+branch sharding scheme — with cost that scales with branch count,
not just project count. A quick test of the Gerrit project's 52 branches
showed the difference concretely: a single Zoekt index covering all 52
branches was about 1.1 GB and took about 40 seconds to build, tagging each
result with the branches that contain it and supporting `branch:<name>`
query filtering natively; covering the same branches in OpenGrok required
a full working-tree checkout per branch, about 407 MB each. OpenGrok's
genuine strength is its code browser (cross-references, jump-to-definition,
annotate, history), but Gitiles already renders files, blame, and history
while enforcing Gerrit's ACLs natively, so this design uses Zoekt for
search and links results out to Gitiles for browsing instead of adopting
or ACL-wrapping a second browser. Zoekt is also licensed Apache 2.0, the
same as Gerrit, versus OpenGrok's CDDL.
