...
Retrive `Account.Id` from a lookup in `AccountBy{Email/ExternalId}`. Retrieve the Account from the AccountCache. If this
...
On Google's infrastructure (this applies also to open-source installations), users are spending tens of hours / day waiting for Gerrit to return a request while Gerrit is reading external IDs. This is not acceptable.
