...
3. If necessary, update the issue summary to be clear.
4. Set label `Security` if it's a security or privacy issue.
5. Close incomplete issues as `Incomplete`.
6. Close spam issues as `Invalid`, flag them as spam and then delete them.
7. Check if the issue has been reported before and close it as `Duplicate` if
   possible.
8. Check if reproduction steps are present and clear. If not, ask the reporter
   to provide them and set the status to `AwaitingInformation`.
9. Set the status to `Accepted` and add the label `Triaged-Yes` when the
   triaging is done.
