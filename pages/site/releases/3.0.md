...
Download: **[3.0.12](https://gerrit-releases.storage.googleapis.com/gerrit-3.0.12.war)**
...
Documentation: **[3.0.12](https://gerrit-documentation.storage.googleapis.com/Documentation/3.0.12/index.html)**
...
* Breaking Changes
  ...
  * Logging: `--console-log`-flag of `gerrit.war daemon` respects `log.textLogging`
    ...
    This change leads to a change in the default behaviour of the `--console-log`
    flag. Since `log.textLogging` in the `gerrit.config` is `true` by default,
    using the `--console-log`-flag will now by default also write logs to the
    `error_log`-file in addition to stderr. This can be avoided by setting
