  ...
  * [Issue 287484350](https://issues.gerritcodereview.com/issues/287484350):
  Introduce NONE pagination type and its support in Lucene indexing backend
    ...
    * wrong entries status (when changes are switching
    ...
    For example any incoming calls that imply the use of the change
    indexing search, such as a simple Git clone, could miss some refs
    or fail because of duplicates in the result set

    There is a [workaround](https://gerrit-review.googlesource.com/c/gerrit/+/349995)
    in place for de-duplicating changes and mitigating the issue, however,
    it does not resolve the inherent inconsistency associated with the
    missing ones.
