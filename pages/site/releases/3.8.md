...
Download: **[3.8.2](https://gerrit-releases.storage.googleapis.com/gerrit-3.8.2.war)**
...
Documentation: **[3.8.2](https://gerrit-documentation.storage.googleapis.com/Documentation/3.8.2/index.html)**
...
* Breaking changes
  ...
  * [Issue 40015585](https://issues.gerritcodereview.com/issues/40015585):
  Breaking change: project based submetric name is sanitized with more rules than just changing `/` to `_`
    ...
    Introduce a dedicated Field builder that is meant
    to be used to extract value from project name (called
    Field.ofProjectName). Metrics impacted are the ProjectName. An example is the name
    being escaped. Before "jgit_block_cache_cache_used_per_repository__var_gerrit_git_some__project"
    after "jgit_block_cache_cache_used_per_repository__0x2F_var_0x2F_gerrit_0x2F_git_0x2F_some_0x2F_project".
...
* New Features
  ...
  * [Issue 287484350](https://issues.gerritcodereview.com/issues/287484350):
  Introduce NONE pagination type and its support in Lucene indexing backend
    ...
    for Lucene.
    A corresponding [change](https://gerrit-review.googlesource.com/c/modules/index-elasticsearch/+/382034) in the elasticsearch libModule,
    allows throwing an IAE to highlight that the backend
    doesn't support NONE pagination type.

    API pagination is not affected by this change and is
    always preserved.

    Pagination could introduce some problems in the result set,
    when moving from one page to the next of the index results:
    ...
    * wrong entries status (when changes are switching
    ...
    For example any incoming calls that imply the use of the change indexing search,
    such as a simple Git clone, could miss some refs or fail because of duplicates
    in the result set

    There is a [workaround](https://gerrit-review.googlesource.com/c/gerrit/+/349995) in 3.8 and later, however, it will not work for
    the missing or inconsistent entries.
    For this case, a workaround [2] has been put in place in master
    and 3.8, however, not resolve the issue of the missing refs in the results.
...
* Documentation fixes
  ...
  * [40014489](https://gerrit-review.googlesource.com/c/gerrit/+/40014489):
