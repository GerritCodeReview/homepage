...
Download: **[3.10.0](https://gerrit-releases.storage.googleapis.com/gerrit-3.10.0.war)**

Documentation: **[3.10.0](http://gerrit-documentation.storage.googleapis.com/Documentation/3.10.0/index.html)**
...
> there is a need to rollback as already existing indxes are affected.
...
The zero-downtime upgrade of multi-site setup (high-availability flavor is
described in 3.9
[release notes](https://www.gerritcodereview.com/3.9.html#online-upgrade-with-zero-downtime))
consists of the following steps ([demo](https://youtu.be/vK5qsb0W7q4)):

1. Have Gerrit servers running v3.9 in multi-site configuration, healthy and
   able to handle the incoming traffic properly.
2. Set `gerrit.experimentalRollingUpgrade` to `true` in `gerrit.config` on all
   Gerrit primaries.
3. Set one of the Gerrit servers to unhealthy.
4. Shutdown the Gerrit server, backup indexes in case a rollback is required,
   update gerrit.war and plugins to v3.10.x, call init and start Gerrit again.
...
8. Remove `gerrit.experimentalRollingUpgrade` from `gerrit.config` on all
   Gerrit primaries.
