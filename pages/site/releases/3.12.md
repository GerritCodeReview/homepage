...
* Security Fixes
  ...
  * [Issue 520369774](https://issues.gerritcodereview.com/issues/520369774):
  Introduce `auth.httpTrustedProxyNetworks` for securing HTTP auth.
    ...
    Introduces the `auth.httpTrustedProxyNetworks` configuration setting to harden HTTP authentication
    security. Administrators can explicitly define the IP networks of trusted reverse proxies,
    preventing unauthorized identity spoofing from untrusted sources and ensuring identity headers are
    only honored when originating from a validated proxy.
...
* Bug Fixes
  ...
  * [Change 443201](https://gerrit-review.googlesource.com/c/gerrit/+/443201):
  WorkQueue: Avoid running more tasks than allowed.
    ...
    parked-task core-pool sizes. This fix improves the overall stability and reliability of background
    task execution and thread management within the server.
  ...
  * [Issue 514855585](https://issues.gerritcodereview.com/issues/514855585):
  AccountManager: Backfill missing username external ID on login.
    ...
    username external ID when a user logs into the system, ensuring proper consistency in the account
    identity mapping and preventing potential login or permission glitches.
  ...
  * [Change 578041](https://gerrit-review.googlesource.com/c/gerrit/+/578041):
  Fix full index scan caused by empty `OR` predicate.
    ...
    Addresses an issue where an index query with an `OR` predicate containing zero conditions caused a
    full scan of all changes, leading to high load and memory consumption. It now checks the specific
    condition and returns an empty result immediately.
...
* Enhancements
  ...
  * [Issue 522635656](https://issues.gerritcodereview.com/issues/522635656):
  Allow removing inactive users from groups.
    ...
    Provides administrators and group owners the ability to remove inactive users from Gerrit groups.
    This enhancement helps maintain cleaner access control lists, prevents legacy accounts from
    cluttering group memberships, and improves overall access auditing.
  ...
  * [Change 583241](https://gerrit-review.googlesource.com/c/gerrit/+/583241):
  Introduce filtering ability for change cleanup.
    ...
    administrators greater flexibility and precision when defining the specific criteria for filtering
    and cleaning up old abandoned changes.
