...
Download: **[3.12.10](https://gerrit-releases.storage.googleapis.com/gerrit-3.12.10.war)**
| [3.12.9](https://gerrit-releases.storage.googleapis.com/gerrit-3.12.9.war)
| [3.12.8](https://gerrit-releases.storage.googleapis.com/gerrit-3.12.8.war)
| [3.12.7](https://gerrit-releases.storage.googleapis.com/gerrit-3.12.7.war)
| [3.12.6](https://gerrit-releases.storage.googleapis.com/gerrit-3.12.6.war)
| [3.12.5](https://gerrit-releases.storage.googleapis.com/gerrit-3.12.5.war)
| [3.12.4](https://gerrit-releases.storage.googleapis.com/gerrit-3.12.4.war)
...
Documentation: **[3.12.10](https://gerrit-documentation.storage.googleapis.com/Documentation/3.12.10/index.html)**
| [3.12.9](https://gerrit-documentation.storage.googleapis.com/Documentation/3.12.9/index.html)
| [3.12.8](https://gerrit-documentation.storage.googleapis.com/Documentation/3.12.8/index.html)
| [3.12.7](https://gerrit-documentation.storage.googleapis.com/Documentation/3.12.7/index.html)
| [3.12.6](https://gerrit-documentation.storage.googleapis.com/Documentation/3.12.6/index.html)
| [3.12.5](https://gerrit-documentation.storage.googleapis.com/Documentation/3.12.5/index.html)
| [3.12.4](https://gerrit-documentation.storage.googleapis.com/Documentation/3.12.4/index.html)
...
to 10.1.0, all the indexes versions have been increased therefore requiring a reindex:
...
Gerrit v3.12.x supports zero-downtime upgrade from Gerrit v3.11 or v3.10 when configured
using either
...
During reindexing, indexes rewritten with the newer Lucene version (10.1.0) are
no longer readable by the previous one.
...
* [Change 447745](https://gerrit-review.googlesource.com/c/gerrit/+/447745)
  ...
  This changes the log format and requires any parsing logic that consume
  `sshd_log` or `httpd_log` entries to be updated accordingly.
...
* Security Fixes
  ...
  * [Issue 540877705](https://issues.gerritcodereview.com/issues/540877705)
  ...
  Restrict user-provided regular expressions with `regex.allowedGroup`.
    ...
    Administrators can now [configure](https://gerrit-documentation.storage.googleapis.com/Documentation/3.12.10/config-gerrit.html#regex.allowedGroup)
    which groups may submit regular expression operands, limiting expensive automaton compilation
  ...
  * [Issue 539032891](https://issues.gerritcodereview.com/issues/539032891):
  Filter non-visible merged changes from `submitted_together`.
    ...
    The `submitted_together` endpoint now applies change visibility checks to merged changes that
  ...
  * [Issue 545820925](https://issues.gerritcodereview.com/issues/545820925):
  Fix `check.submit_requirement` API group visibility leak.
    ...
    Ad hoc submit requirement checks via [check.submit_requirement API](https://gerrit-documentation.storage.googleapis.com/Documentation/3.12.10/rest-api-changes.html#check-submit-requirement)
...
* Performance improvements
  ...
  * [Change 613681](https://gerrit-review.googlesource.com/c/gerrit/+/613681)
  ...
  Avoid full index scans for nested empty query predicates.

    The change index rewriter now keeps `none()` and empty `OR` short-circuits as
    `ChangeDataSource` instances, preventing nested queries from falling back to scans over all
    open and closed changes.

  * [Change 621781](https://gerrit-review.googlesource.com/c/gerrit/+/621781):
  Validate only changed submit requirements in project configuration updates.

    Gerrit now skips validation for unchanged submit requirements when `project.config` is pushed,
    reducing repeated validation cost for projects with many existing requirements.

* Frontend changes

  * [Change 620821](https://gerrit-review.googlesource.com/c/gerrit/+/620821):
  Show server-side errors when adding GPG keys fails.

    The settings UI now displays the backend error message when a submitted GPG key is invalid,
    expired, or does not certify one of the user's identities.

* JGit changes

  * [Change 610261](https://gerrit-review.googlesource.com/c/gerrit/+/610261):
  Bump JGit to 2f8bc42fa.

    You can retrieve the full list of changes with:
    ```
    git log --oneline --no-merges f2c3bdde6..2f8bc42fa
    ```
...
* Documentation Updates
  ...
  * [Change 608061](https://gerrit-review.googlesource.com/c/gerrit/+/608061):
  Document `Trace-Id` fields in HTTP and SSH logs.
...
* Plugin Updates
  ...
  * `codemirror-editor` plugin (updated to `efc967bbb3`):
...
* Dependency Updates
  ...
  * `icu4j` version `57.1` has replaced `UniversalDetector` for better GBK detection.
...
* Enhancements
  ...
  * [Change 547432](https://gerrit-review.googlesource.com/c/gerrit/+/547432):
  Changed `gerrit.service` to use `gerrit.sh`, so it respects container configuration.
...
* Documentation Updates
  ...
  * [Change 572621](https://gerrit-review.googlesource.com/c/gerrit/+/572621):
  Document `Delete Group` capability in `access-control`.
    ...
    Clarifies that this capability allows deleting internal groups through the REST API. It can be
    granted to non-administrators, and only takes effect when `groups.enableDeleteGroup` is
    enabled.

* Plugin changes

  * Update replication to `4698ef0`

    You can retrieve the full list of changes with:

    ```
    git log --oneline --no-merges b0e7aa6..4698ef0
    ```
