...
Gerrit 3.12 now consumes JGit directly from the stable-7.4 branch.

The [previous release note](#3128) warned that Gerrit stable-3.12 could
not use JGit stable-7.4 and that users upgrading from stable-3.10 or
stable-3.11 would miss some JGit fixes. That limitation has been lifted.

#### JGit Changes

  ```shell
  git log --no-merges --oneline bb309748c..c2188c2a0
  ```

c2188c2a0 (HEAD) junit.http: Extract AppServerBase to dedupe the EE8 AppServer overlay
...
f2c3bdde6 (origin/stable-6.10) Document JGit receive command size options
