...
* Security Fixes
  ...
  * [Issue 540877705](https://issues.gerritcodereview.com/issues/540877705)
  ...
  Restrict user-provided regular expressions with `regex.allowedGroup`.
    ...
    Administrators can now configure which groups may submit regular expression operands, limiting
    expensive automaton compilation for interactive change queries, project/branch/tag listing
    filters, bulk draft deletion, project watches, submit requirements, project notification
    filters, and ACL ref patterns.
  ...
  * [Issue 545820925](https://issues.gerritcodereview.com/issues/545820925):
  Fix `check.submit_requirement` group visibility leak.

    Ad hoc submit requirement checks now run with the caller's group visibility, preventing
    predicates such as `ownerin:<group>` from revealing hidden group membership information.
