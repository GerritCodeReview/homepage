...
### 2.14.12 (in development)

* [Issue 9528](https://bugs.chromium.org/p/gerrit/issues/detail?id=9528):
Allow project-level `receive.maxObjectSizeLimit` to be inherited from parent
project.
  ...
  A new global option `receive.inheritProjectMaxObjectSizeLimit` is added,
  and when this is enabled the project-level setting is inherited from the
  parent. This new setting is disabled by default to keep backwards
