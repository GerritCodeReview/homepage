...
Download: **[3.5.0-rc4](https://gerrit-releases.storage.googleapis.com/gerrit-3.5.0-rc4.war)**

Documentation: **[3.5.0-rc4](https://gerrit-documentation.storage.googleapis.com/Documentation/3.5.0/index.html)**
...
ProjectCache#evict no longer triggers a reindex, consistently with other evict() methods in Gerrit.

  ProjectCache.evict() was also triggering reindex on the project while there was
  no way to just evict the project cache entry. The current method is renamed evictAndReindex()
  and a new method evict() replaces the existing one, triggering only eviction.
