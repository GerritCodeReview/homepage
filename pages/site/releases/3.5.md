...
* New features
  ...
  * [Issue 16565](https://crbug.com/gerrit/16565):
  Introduce cache.threads option to allow custom executors for Caffeine caches.
    ...
    The introduction of `cache.threads` in `gerrit.config` allow to configure
    an explicit separate thread pool which can be tuned and decoupled from the
  ...
  * [Change 353177](https://gerrit-review.googlesource.com/c/gerrit/+/353177):
  Add tsconfig-plugins-base.json to Gerrit TypeScript Plugin API.
    ...
    Adds `tsconfig-plugins-base.json` to the npm package so downstream plugin
    can easily adhere to the same set of TypeScript settings.

  * [Change 351674](https://gerrit-review.googlesource.com/c/gerrit/+/351674):
  SSH review cmd handles lock failures with retries.

    Retrying on lock failures helps mitigating lock failures as there will
    be a contention to lock the notedb ref. Retries were already implemented
  ...
  * [Change 353594](https://gerrit-review.googlesource.com/c/gerrit/+/353594):
  Inserting new patch-sets is retried on lock failures.
    ...
    Retrying on lock failures when creating new patch-sets fails if the notedb
  ...
  * [Change 353175](https://gerrit-review.googlesource.com/c/gerrit/+/353175):
  No more include `BUILD_for_publishing_api_only` file when building npm
...
* Bug Fixes
  ...
  * [Change 355196](https://gerrit-review.googlesource.com/355196):
  Fix the ability to configure JSCH as client implementation in `gerrit.config`.
    ...
    Make the legacy JSCH implementation explicit for being able to configure
  ...
  * [Change 355054](https://gerrit-review.googlesource.com/355054):
  Wrap #mobileSearch in a div.
    ...
    On wikimedia there was an issue where the search button was hidden
    even though the rules execute to make it show.
...
* Dependency Updates
  ...
  Update jgit to 801a56b48a7fe3c6e171073211cc62194184fe79.
  This update from stable-5.9 to stable-6.5 contains the following changes:

  * *TODO: @Matthias can you highlight the noteworthy changes to be mentioned?*
