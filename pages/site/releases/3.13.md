...
Download: **[3.13.0-rc0](https://gerrit-releases.storage.googleapis.com/gerrit-3.13.0-rc0.war)**

Documentation: **[3.13.0-rc0](https://gerrit-documentation.storage.googleapis.com/Documentation/3.13.0/index.html)**
...
- Reindex all indexes:
  ...
  See the
  [reindex](https://gerrit-documentation.storage.googleapis.com/Documentation/3.13.0/pgm-reindex.html)
...
1. Have Gerrit servers running v3.12 in high-availability or multi-site
...
* [Change 464861](https://gerrit-review.googlesource.com/c/gerrit/+/464861):
  Add option to limit the number of auth tokens per account.

  So far accounts could have unlimited tokens configured. Usually there
  should not be a usecase where an account needs hundreds or thousands
  of tokens. On the other hand, allowing unlimited tokens makes it easier
  to spam the notedb, might cause performance issues and causes issues
  with rendering the user settings.

  This change limits the number of tokens per account by default to 10.
  Administrators can adapt this number.

* [Change 458724](https://gerrit-review.googlesource.com/c/gerrit/+/458724):
  Deprecate passwords in external IDs

  A current effort is to move from the HTTP password to tokens with a
  limited lifetime. These have a different storage format. Thus, this
  change deprecates passwords in external IDs.

* [Change 458727](https://gerrit-review.googlesource.com/c/gerrit/+/458727):
  REST API endpoints to manage HTTP passwords are deprecated.

  This also affects corresponding SSH commands, since those use
  REST API classes to apply the changes.
...
* [Change 492103](https://gerrit-review.googlesource.com/c/gerrit/+/492103):
  Return the base that was used when performing a merge in ConflictsInfo (Reland).
...
* [Change 481052](https://gerrit-review.googlesource.com/c/gerrit/+/481052):
  ...
  This allows plugins to add addtional checks before code approval
  actions.
...
* [Change 461981](https://gerrit-review.googlesource.com/c/gerrit/+/461981):
  Allow admins to limit auth token lifetime globally.

  This change allows admins to enforce a maximum lifetime for
  tokens, which helps in enforcing security policies aiming
  to protect access to Gerrit.

* [Change 459866](https://gerrit-review.googlesource.com/c/gerrit/+/459866):
  Allow to generate tokens with the set-account command.

* [Change 459005](https://gerrit-review.googlesource.com/c/gerrit/+/459005):
  Add offline migration tool for migrating HTTP passwords to tokens

  This change adds a tool to migrate HTTP passwords of all accounts
  to authentication tokens offline.
