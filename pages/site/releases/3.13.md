...
* [Issue 459326231](https://issues.gerritcodereview.com/issues/459326231):
  New HTTP tokens are not replicated

  When a new HTTP token is generated, the ref-update events are not fired, leaving the rest
  of the Gerrit cluster nodes potentially inconsistent:
  - replicas do not receive the updated tokens, therefore the new credentials will not be
    propagated and Git operations will fail
  - other primary nodes do not receive the updated tokens and remain out of sync with
    the global-refdb, blocking further updates to the associated user profile

  As a workaround, the missed replication need to be triggered manually using either
  push or pull-replication start commands.
