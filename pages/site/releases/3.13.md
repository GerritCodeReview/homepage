...
Download: **[3.13.9](https://gerrit-releases.storage.googleapis.com/gerrit-3.13.9.war)**
| [3.13.8](https://gerrit-releases.storage.googleapis.com/gerrit-3.13.8.war)
| [3.13.7](https://gerrit-releases.storage.googleapis.com/gerrit-3.13.7.war)
| [3.13.6](https://gerrit-releases.storage.googleapis.com/gerrit-3.13.6.war)
| [3.13.5](https://gerrit-releases.storage.googleapis.com/gerrit-3.13.5.war)
| [3.13.4](https://gerrit-releases.storage.googleapis.com/gerrit-3.13.4.war)
| [3.13.3](https://gerrit-releases.storage.googleapis.com/gerrit-3.13.3.war)
...
Documentation: **[3.13.9](https://gerrit-documentation.storage.googleapis.com/Documentation/3.13.9/index.html)**
| [3.13.8](https://gerrit-documentation.storage.googleapis.com/Documentation/3.13.8/index.html)
| [3.13.7](https://gerrit-documentation.storage.googleapis.com/Documentation/3.13.7/index.html)
| [3.13.6](https://gerrit-documentation.storage.googleapis.com/Documentation/3.13.6/index.html)
| [3.13.5](https://gerrit-documentation.storage.googleapis.com/Documentation/3.13.5/index.html)
| [3.13.4](https://gerrit-documentation.storage.googleapis.com/Documentation/3.13.4/index.html)
| [3.13.3](https://gerrit-documentation.storage.googleapis.com/Documentation/3.13.3/index.html)
...
* [Change 474301](https://gerrit-review.googlesource.com/c/gerrit/+/474301)
  [Change 474303](https://gerrit-review.googlesource.com/c/gerrit/+/474303):
  Remove Robot Comments from Gerrit UI and the backend.

  Robot comments are not supported anymore and users have to move to the
  Checks API.
...
* [Change 477742](https://gerrit-review.googlesource.com/c/gerrit/+/477742):
  Rejects implicit merges on push by default.

  Sites that rely on pushing implicit merges must explicitly allow this behavior
  in [project configuration](https://gerrit-documentation.storage.googleapis.com/Documentation/3.13.8/config-project-config.html#receive.rejectImplicitMerges)
  before upgrading, otherwise affected pushes will be rejected.
...
* Security Fixes
  ...
  * [Issue 541287630](https://issues.gerritcodereview.com/issues/541287630)
    [CVE-2026-87721](https://nvd.nist.gov/vuln/detail/cve-2026-87721):
  Prevent denial-of-service in the query parser.

    Gerrit now enables ANTLR memoization for query parsing, avoiding exponential backtracking on
    deeply nested valid or malformed queries without changing the parsed query semantics.

  * [Issue 538883789](https://issues.gerritcodereview.com/issues/538883789)
    [CVE-2026-87720](https://nvd.nist.gov/vuln/detail/cve-2026-87720):
  Reject project names with repeated terminal `.git` suffixes.

    Gerrit now rejects REST, SSH and Git-over-HTTP requests for project names such as
    `P.git.git`, preventing alternate logical project names from reusing the same physical
    repository with stale project-state authorization.

  * [Issue 540877705](https://issues.gerritcodereview.com/issues/540877705)
    [CVE-2026-87722](https://nvd.nist.gov/vuln/detail/cve-2026-87722):
  Restrict user-provided regular expressions with `regex.allowedGroup`.

    Administrators can [restrict](https://gerrit-documentation.storage.googleapis.com/Documentation/3.12.10/config-gerrit.html#regex.allowedGroup)
    which groups can use regular expression operands, limiting expensive automaton compilation
    for interactive change queries, project/branch/tag listing filters, bulk draft deletion,
    project watches, submit requirements, project notification filters, and ACL ref patterns.

  * [Issue 539032891](https://issues.gerritcodereview.com/issues/539032891):
  Filter non-visible merged changes from `submitted_together`.

    The `submitted_together` endpoint applies change visibility checks to merged changes that
    share a submission ID, preventing callers from learning about changes they cannot see.

  * [Issue 545820925](https://issues.gerritcodereview.com/issues/545820925):
  Fix `check.submit_requirement` API group visibility leak.

    Submit requirement checks via [check.submit_requirement API](https://gerrit-documentation.storage.googleapis.com/Documentation/3.12.10/rest-api-changes.html#check-submit-requirement)
    now run with the caller's group visibility, preventing predicates such as `ownerin:<group>`
    from revealing hidden group membership information.

* Bug Fixes

  * [Change 621301](https://gerrit-review.googlesource.com/c/gerrit/+/621301):
  Preserve regex path query parse errors.

    Regex path predicates now propagate the underlying parse failure reason, helping REST, SSH, and
    submit requirement validation callers report actionable errors for invalid path patterns.

  * [Change 613482](https://gerrit-review.googlesource.com/c/gerrit/+/613482):
  WorkQueue no longer drops queued tasks when their `toString()` method throws.

    Gerrit now falls back to the task class name when naming worker threads,
    ensuring background work still executes even if a caller-provided runnable
    has a faulty string representation.

  * [Change 616881](https://gerrit-review.googlesource.com/c/gerrit/+/616881):
  Fix internal server errors when pushing a new patch set to a change that was
  closed concurrently.

    Gerrit now skips post-update processing after the replacement operation has
    already recorded a rejection reason, preserving the expected user-facing
    rejection instead of surfacing a `NullPointerException`.

  * [Change 616861](https://gerrit-review.googlesource.com/c/gerrit/+/616861):
  Fix `NullPointerException` when a revision has no branch stored in NoteDb.

    Revision rendering now falls back to the change destination branch, which
    restores pushing new patch sets and editing commit messages for changes with
    sparse or failed patch set histories.

  * [Change 620821](https://gerrit-review.googlesource.com/c/gerrit/+/620821):
  Display server-side errors when adding a GPG key from the settings UI.

    Users now see the validation error returned by Gerrit, for example when a
    key is invalid, expired, or does not certify any identity on the account.

* Performance improvements

  * [Change 621182](https://gerrit-review.googlesource.com/c/gerrit/+/621182):
  Compile path regex predicates when queries are parsed.

    Gerrit now builds the path regex searcher once during predicate construction, reporting invalid
    patterns immediately and avoiding repeated compilation while matching changes.

  * [Change 613681](https://gerrit-review.googlesource.com/c/gerrit/+/613681):
  Fix a full index scan when a `none()` predicate is nested inside an `AND`
  within an `OR`.

    Gerrit now keeps the optimized empty-result query as a `ChangeDataSource`,
    avoiding extremely slow change queries that should return no results.

  * [Change 613701](https://gerrit-review.googlesource.com/c/gerrit/+/613701):
  Fix a full index scan when an empty `OR` predicate is nested inside an `AND`
  within an `OR`.

    Empty query branches are now wrapped as indexed empty-result sources, so
    the query rewriter can preserve the short-circuit and avoid scanning all
    open and closed changes.

  * [Change 621781](https://gerrit-review.googlesource.com/c/gerrit/+/621781):
  Only validate submit requirements when they changed.

    Pushing project configuration no longer revalidates unchanged submit
    requirements on every update, reducing repeated regex and query validation
    cost for large or frequently edited project configurations.

* UI Fixes

  * [Change 614661](https://gerrit-review.googlesource.com/c/gerrit/+/614661):
  Preserve edit mode and the selected base when changing the diff patch range.

    Users editing a change can now switch the displayed base patch set without
    silently leaving edit mode, including when inspecting merge commits against
    a parent base.

* Documentation Updates

  * [Change 608061](https://gerrit-review.googlesource.com/c/gerrit/+/608061):
  Document that HTTP and SSH logs include the request `Trace-Id`.

  * [Change 623261](https://gerrit-review.googlesource.com/c/gerrit/+/623261):
  Correct the documented success response code for the HTTP Password REST API.

    The compatibility endpoint for creating authentication tokens returns
    `201 Created` on success.

* JGit changes

  * [Change 610261](https://gerrit-review.googlesource.com/c/gerrit/+/610261):
  Bump JGit to `49097e98f`.

    You can retrieve the full list of changes with:

    ```
    git log --oneline --no-merges 94fe3e89f..49097e98f
    ```

    Notable changes are:
    - `2f8bc42fa` Ensure the `SnapshottingRefDirectory` shares the `packedRefsRefresher`.

### 3.13.8

* Bug Fixes

  * [Issue 525330157](https://issues.gerritcodereview.com/issues/525330157)
    [Change 600721](https://gerrit-review.googlesource.com/c/gerrit/+/600721):
  Fix review responses that could show a stale change status after a synchronous post-review update.

    If a review listener or plugin modified the change during post-review processing,
    Gerrit could return outdated `ChangeInfo` in the review response. Gerrit now refreshes
    the change data when needed so the returned status reflects the final change state.

  * [Change 600361](https://gerrit-review.googlesource.com/c/gerrit/+/600361):
  Fix missing changes from `byProjectGroups` when group count exceeds the index batch size.

    Prevents changes from being incorrectly dropped when `byProjectGroups` queries are split
    into multiple index batches.

* Performance improvements

  * [Change 599401](https://gerrit-review.googlesource.com/c/gerrit/+/599401):
  Speed up `ParentDataProvider` merged check with `RevWalk`.

    Improves merged-check performance by using a more efficient JGit API, reducing the
    time spent in `ParentDataProvider.get()` on repositories with many refs and heads.

* Plugin Updates

  * `codemirror-editor` plugin (updated to `40606b33d0`):

    * [Issue 393149327](https://issues.gerritcodereview.com/issues/393149327)
      [Change 602744](https://gerrit-review.googlesource.com/c/plugins/codemirror-editor/+/602744):
    Preserve line terminator upon edits.

      Prevents DOS-style line endings from being unintentionally rewritten to Unix-style
      line endings when editing files through the CodeMirror editor.

* JGit changes

  * [Change 604821](https://gerrit-review.googlesource.com/c/gerrit/+/604821):
  Bump JGit to 94fe3e89f

    You can retrieve the full list of changes with:
    ```
    git log --oneline --no-merges bb309748c..94fe3e89f
    ```

    Notable changes are:
    - `41c2f03b2` Introduce `fetch.autogc` to disable concurrent GC upon fetches
    - `189b33dcb` FetchProcess: Report lock failure when destination ref changes
...
* Plugin Updates
  ...
  * `replication` plugin (updated to `7a8f3429ed`):
...
> **JGit Note**: `jgit` has been updated to `f2c3bdde6` in `stable-3.10` and `stable-3.11`
> (currently EOL) but cannot be merged up to `stable-3.12` and `stable-3.13` because
> of build and source incompatibilities with Gerrit.
>
> You can see the full list of **changes that are missing** when upgrading from stable-3.10/3.11
> to v3.12.x:
>
>  ```bash
>  git log --oneline --no-merges bb309748c..f2c3bdde6
>  ```
>
>  * 41c2f03b2: Introduce `fetch.autogc` to disable concurrent GC upon fetches.
>  * 189b33dcb: FetchProcess: Report lock failure when destination ref changes.
...
* Dependency Updates
  ...
  * `bazel` updated to version `7.7.1`.
...
* Bug Fixes
  ...
  * [Change 566421](https://gerrit-review.googlesource.com/c/gerrit/+/566421):
  Fixed `RequireSslFilter` redirect when `canonicalWebUrl` is HTTPS.

    Fixed malformed HTTP-to-HTTPS redirects when canonicalWebUrl is set to an HTTPS URL, avoiding
    double slashes in the generated redirect path.

* Enhancements

  * [Change 547432](https://gerrit-review.googlesource.com/c/gerrit/+/547432):
  Changed `gerrit.service` to respect container configuration.

  * [Issue 502757952](https://issues.gerritcodereview.com/issues/502757952)
    [Change 573501](https://gerrit-review.googlesource.com/c/gerrit/+/573501):
  Allow disabling AI review prompt.

    Keep the AI review prompt enabled by default as shipped in 3.13, but
    allow administrators to [disable](https://gerrit-documentation.storage.googleapis.com/Documentation/3.13.6/config-gerrit.html#experiments.ui_feature_get_ai_prompt)
    it.

* UI Fixes

  * [Change 564501](https://gerrit-review.googlesource.com/c/gerrit/+/564501):
  Include `SKIP_DIFFSTAT` in preloaded submit requirements to avoid a redundant API call during
  change detail loading.

  * [Change 569921](https://gerrit-review.googlesource.com/c/gerrit/+/569921):
  `gr-access-section`: fix displaying the name for `removeLabel-` permissions.

  * [Change 570741](https://gerrit-review.googlesource.com/c/gerrit/+/570741):
  Fix properly closing a popup under a plugin.

    Closing a plugin popup now removes the old popup element from the DOM instead of leaving unused
    elements behind.

  * [Change 571961](https://gerrit-review.googlesource.com/c/gerrit/+/571961):
  `gr-menu-editor`: Allow changing URL target after saving.

  * [Change 571962](https://gerrit-review.googlesource.com/c/gerrit/+/571962):
  `gr-change-table-editor`: Improve detection of unsaved changes.

  * [Change 572441](https://gerrit-review.googlesource.com/c/gerrit/+/572441):
  `gr-menu-editor`: Support changing the name and URL of an entry.

  * [Change 542241](https://gerrit-review.googlesource.com/c/gerrit/+/542241):
  Fixed `Delete vote` button not checking removable_labels and therefore
  not respecting the ACLs for displaying it.

  * [Change 573581](https://gerrit-review.googlesource.com/c/gerrit/+/573581):
  Fix menu editor toggling link target checkbox on existing menu items.

* Documentation Updates

  * [Change 572621](https://gerrit-review.googlesource.com/c/gerrit/+/572621):
  Document `Delete Group` capability in `access-control`.

    Clarifies that the capability allows deleting internal groups through the REST API, can be
    granted to non-administrators, and only takes effect when `groups.enableDeleteGroup` is
    enabled.

* Plugins

  * Update replication to `163612b`

    You can retrieve the full list of changes with:

    ```
    git log --oneline --no-merges b62454c..163612b
    ```

    Notable changes are:

    * fbd8be0 Allow configuring the reflog for newly created repositories with `remote.NAME.storeRefLog`.
    * 24b0f4b Add `--remote` switch to replication start command

### 3.13.5

* Bug Fixes

  * [Change 563501](https://gerrit-review.googlesource.com/c/gerrit/+/563501):
  Fix infinite loading and group visibility in Gerrit access pages.

    When a user lacks permission to view a group, the backend filters out the GroupInfo
    and getGroupConfig REST endpoints return an error or empty config, which caused
    the UI to freeze indefinitely on 'Loading...' in the Group Options or
    displaying the group's UUID, which was opaque to users, in the Repository Access Page.

### 3.13.4

* Security Fixes

  * [Issue 486131256](https://issues.gerritcodereview.com/issues/486131256)
    [CVE-2026-2725](https://www.cve.org/CVERecord?id=CVE-2026-2725):
  Create/update change with submit option bypasses ACLs when used with the `change.submitWholeTopic = true`.

    A Gerrit user with permissions to create/update changes with the submit option, could
    submit other changes when `change.submitWholeTopic=true` is set on `gerrit.config`.
    Submit to a Gerrit branch can force-merge any visible change by linking it via a shared topic.

* Enhancements

  * [Change 555021](https://gerrit-review.googlesource.com/c/gerrit/+/555021):
  Allow configuring additional recipients for authentication token expiry emails.

    This lets administrators include extra notification targets for service
    users and other accounts that do not have a directly monitored mailbox.

* UI Fixes

  * [Change 554183](https://gerrit-review.googlesource.com/c/gerrit/+/554183):
  Prevent page reload when clicking the documentation icon in search autocomplete.

    The documentation icon button now uses a non-submit type, so clicking it
    no longer triggers accidental form submission and full-page refresh.
...
* Bug Fixes
  ...
  * [Change 526705](https://gerrit-review.googlesource.com/c/gerrit/+/526705):
    Allow admins to see all refs on fetch and when creating branches.

    Admins can now perform `git fetch` on references even without explicit read permissions.
    Additionally, they can create branches on commits that are only reachable from branches
    for which they lack explicit read access. This ensures that admins, who could already see
    these refs via the REST API, can also interact with them through standard Git operations.
  ...
  * [Change 524669](https://gerrit-review.googlesource.com/c/gerrit/+/524669):
  ...
  * [Change 534541](https://gerrit-review.googlesource.com/c/gerrit/+/534541):
    Enable submit action on project config only changes if project is read-only or hidden.

    Previously, when a project was read-only or hidden, the `Submit` button was
    not displayed for changes that only modified the project configuration,
    forcing users to use the REST API. The submit action is now correctly
    enabled in the UI for these changes.

  * [Change 537701](https://gerrit-review.googlesource.com/c/gerrit/+/537701):
    Fix computation of commit message body when second line is not a blank line.

  * [Change 537681](https://gerrit-review.googlesource.com/c/gerrit/+/537681):
    Improve footer parsing by moving metadata found at the end of the body into
    the footer section. This handles cases where users accidentally include new
    lines between footers.

  * [Change 537661](https://gerrit-review.googlesource.com/c/gerrit/+/537661):
    Fix commit message wrapping for body paragraphs containing colons.

    Body paragraphs containing colons are now correctly identified and wrapped
    to the 72-character limit, instead of being incorrectly treated as
    unwrapped metadata footers.
...
* Enhancements
  ...
  * [Change 526341](https://gerrit-review.googlesource.com/c/gerrit/+/526341):
  ...
  * [Change 520401](https://gerrit-review.googlesource.com/c/gerrit/+/520401):
    Add before commit message event

    This allows plugins to add addtional checks before commit message edits.

* UI Fixes

  * [Issue 461541278](https://issues.gerritcodereview.com/issues/461541278):
    Fix context path being omitted in "Your" menu links when opening in a new tab.

    When `gerrit.canonicalWebUrl` included a context path (e.g., `/gerrit/`), custom
    menu items or items configured to open in a new tab would incorrectly omit
    the context path from the generated URL. The `gr-dropdown` component now
    correctly prepends the base URL in these cases.

  * [Change 527901](https://gerrit-review.googlesource.com/c/gerrit/+/527901):
    Rename `Create New` to `Create Group`.

  * [Change 526684](https://gerrit-review.googlesource.com/c/gerrit/+/526684):
    Rename `Create New` to `Create (Branch|Tag)`.

  * [Change 527762](https://gerrit-review.googlesource.com/c/gerrit/+/527762):
    Rename `Create New` to `Create Repository`.

    Also move it to the right of `Create Change`.

  * [Change 526681](https://gerrit-review.googlesource.com/c/gerrit/+/526681):
    Prevent title from overflowing into next line on small width

* Performance improvements

  * [Change 537662](https://gerrit-review.googlesource.com/c/gerrit/+/537662):
