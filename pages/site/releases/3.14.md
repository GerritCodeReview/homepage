...
* Security Fixes
  ...
  * [Issue 520369774](https://issues.gerritcodereview.com/issues/520369774):
    ...
    When using `auth=HTTP` or `auth=HTTP_LDAP`, Gerrit reads the
    authenticated username from an HTTP header, which can be spoofed if
    Gerrit listens on an external interface over plain HTTP behind a
    proxy. The new `auth.httpTrustedProxyNetworks` setting restricts
    header-based username extraction to a configured set of trusted
    proxy CIDRs, mitigating HTTP header spoofing.
...
* Bug Fixes
  ...
  * [Issue 514817934](https://issues.gerritcodereview.com/issues/514817934):
    ...
    Reverts the server-info/version caching change that broke the
    `download-commands` plugin and caused the SSH download URL to show
    the wrong user id or be invisible.

  * [Issue 515480627](https://issues.gerritcodereview.com/issues/515480627):
    Stop the mergeable endpoint returning `409 Conflict` for merged and
    abandoned changes.

    Checking the mergeability of an already-merged or abandoned change
    now returns `200 OK` with a `MergeableInfo` payload instead of a
    `409 Conflict`. The payload sets `commit_merged`/`content_merged`
    appropriately so callers can distinguish an already-integrated
    change from one that is mergeable but not yet integrated.

  * [Issue 522635656](https://issues.gerritcodereview.com/issues/522635656):
    Allow removing inactive users from groups.

    Group member deletion inputs are now resolved with inactive accounts
    included, so removing an already-inactive member no longer fails
    with `422`.

  * [Issue 514855585](https://issues.gerritcodereview.com/issues/514855585):
    Backfill the missing `username:` external ID on login.

    When an account has a `gerrit:` external ID but no `username:`
    external ID, SSH key and HTTP password authentication were silently
    broken. `AccountManager` now backfills the missing `username:`
    external ID on login when the auth provider supplies a username.
  ...
  * [Change 586121](https://gerrit-review.googlesource.com/c/gerrit/+/586121):
    ...
    Encoding detection now uses ICU4J's `CharsetDetector`, improving
    accuracy for GBK and other encodings.

  * [Change 583841](https://gerrit-review.googlesource.com/c/gerrit/+/583841):
    Fix `group_expansion_latency` LDAP metric not being recorded in some
    cases.

  * [Change 443201](https://gerrit-review.googlesource.com/c/gerrit/+/443201):
    Avoid running more `WorkQueue` tasks than allowed.

    Fixes core-pool-size races around parked tasks so the parked slot is
    decremented exactly once when a parked task is unparked or
    cancelled, preventing transient over-concurrency.
...
* UI Fixes
  ...
  * [Change 582201](https://gerrit-review.googlesource.com/c/gerrit/+/582201):
    Only show the "Create AI Review Prompt" action to logged-in users.

    The action and dialog are now hidden for anonymous users, since
    submitting AI-generated review feedback requires login anyway.
...
* Enhancements
  ...
  * [Change 588201](https://gerrit-review.googlesource.com/c/gerrit/+/588201):
    Add a `query` option for flexible `changeCleanup` filtering.

    Operators can now express any cleanup constraint directly in
    Gerrit's query language, for example:

    ```
    [changeCleanup]
      query = limit:100 age:5w -project:some/repo -hashtag:keep-alive
    ```

* Performance improvements

  * [Change 578041](https://gerrit-review.googlesource.com/c/gerrit/+/578041):
    Short-circuit empty `OrPredicate` (and `none()` children of
    `AndPredicate`) in the index rewriter.

    An `OrPredicate` with zero children previously fell through to the
    general rewrite path and triggered a full scan of all changes in the
    index; it now resolves to `none()` immediately.

  * [Change 579121](https://gerrit-review.googlesource.com/c/gerrit/+/579121):
    Avoid post-filtering for plain label index queries.

    Label queries without a group or count constraint now trust the
    exact index result directly instead of re-evaluating the full
    predicate tree, avoiding unnecessary NoteDB reads.
