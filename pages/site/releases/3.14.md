...
* Bug Fixes
  ...
  * [Issue 514817934](https://issues.gerritcodereview.com/issues/514817934):
    Fix wrong or missing SSH download URL in the download-commands
    plugin.
  ...
  * [Change 583861](https://gerrit-review.googlesource.com/c/gerrit/+/583861):
    Fix a typo in the `gerrit.sh` debug flags.

* UI Fixes

  * [Change 582201](https://gerrit-review.googlesource.com/c/gerrit/+/582201):
    Hide "Create AI Review Prompt" action for anonymous traffic.

    The action and dialog are now hidden for anonymous users, since
...
* Plugins
  ...
  * `replication` plugin (updated to `3e623c750`):
    ...
    * [Change 568401](https://gerrit-review.googlesource.com/c/plugins/replication/+/568401):
...
* Dependency Updates
  ...
  * Embedded `jetty` server updated from `9.4` to `12.1.10`. Jetty 9 has
    reached end-of-life and is no longer published to Maven Central
    after January 2026; the upgrade preserves compatibility with
    existing `javax.servlet`-based plugins via Jetty's EE8 adapter
    modules and requires no plugin migration to Jakarta EE.
