...
SSH/HTTP download urls provided by the download-commans plugin show wrong userid or are
not visible at all.

  The [Change 565681](https://gerrit-review.googlesource.com/c/gerrit/+/565681)
  has introduced a new performance improvement to the rendering of the home page
  by caching the server info across incoming calls. The server info also contains
  the download URLs that could be user-specific, hence introducing the regression.

  The new cache introduced must be disabled, so that the server info can be
  recalculated at every call, with the drawback of disabling also the performance
  improvement.
  ...
  > **NOTE**: `server_config` is a non-loading Cache; `memoryLimit = 0` relies on
  > Caffeine's weight-based eviction (entries evicted asynchronously
  > immediately after insertion) rather than the cleaner
  > PassthroughLoadingCache shortcut that applies to LoadingCache instances.
  > There is a small race window where a stale entry may be returned before
  > async eviction, but in practice this is sufficient to mitigate this
  > issue.
...
* Security Fixes
  ...
  * [Issue 520369774](https://issues.gerritcodereview.com/issues/520369774):
    Introduce `auth.httpTrustedProxyNetworks` to mitigate HTTP header spoofing
    when using HTTP/HTTP_LDAP authentication behind reverse proxies.

    Introduces the `auth.httpTrustedProxyNetworks` configuration setting to
    harden HTTP authentication security. Administrators can explicitly define
    the IP networks of trusted reverse proxies, preventing unauthorized identity
    spoofing from untrusted sources and ensuring identity headers are only
    honored when originating from a validated proxy.
...
* Bug Fixes
  ...
  * [Change 443201](https://gerrit-review.googlesource.com/c/gerrit/+/443201):
    ...
    Addresses a concurrency issue within the WorkQueue that could lead to race
    conditions affecting parked-task core-pool sizes. This fix improves the
    overall stability and reliability of background task execution and thread
  ...
  * [Issue 514855585](https://issues.gerritcodereview.com/issues/514855585):
    ...
    Fixes an edge case involving missing external IDs. Gerrit will now
    automatically backfill the username external ID when a user logs into the
    system, ensuring proper consistency in the account identity mapping and
    preventing potential login or permission glitches.

  * [Change 583841](https://gerrit-review.googlesource.com/c/gerrit/+/583841):
    Fix `group_expansion_latency` metric not recorded.

    Resolves an issue where the `group_expansion_latency` metric was
    occasionally dropped.

  * [Change 583861](https://gerrit-review.googlesource.com/c/gerrit/+/583861):
    Fix typo in `gerrit.sh` debug flags.

    Corrects a minor typographical error in the runner script to ensure proper
    execution.

  * [Issue 515480627](https://issues.gerritcodereview.com/issues/515480627):
    Return `MergeableInfo` for merged or abandoned changes.

    Updates the REST API to successfully calculate and return `MergeableInfo`
    even when a change has been abandoned or merged, rather than returning HTTP
    status 409. This aligns the API behavior with the documented payload and
    status codes.

  * [Change 578041](https://gerrit-review.googlesource.com/c/gerrit/+/578041):
    Fix full index scan caused by empty `OR` predicate.

    Addresses an issue where an index query with an `OR` predicate containing
    zero conditions caused a full scan of all changes, leading to high load and
    memory consumption. It now checks the specific condition and returns an
    empty result immediately.

  * [Issue 514817934](https://issues.gerritcodereview.com/issues/514817934):
    Fix wrong or missing SSH download URL caused by server info caching.

    Reverts the server-info and version caching that improved initial load
    latency but broke the `download-commands` plugin, causing the SSH download
    URL to show the wrong user id or to be missing entirely.

  * [Change 586121](https://gerrit-review.googlesource.com/c/gerrit/+/586121):
    Replace `UniversalDetector` with ICU4J for better GBK detection.

    The previous encoding detection using `UniversalDetector` was inaccurate for
    GBK; it is now replaced with ICU4J's `CharsetDetector`, which provides
    better support and accuracy for GBK and other encodings.
...
* UI Fixes
  ...
  * [Change 582201](https://gerrit-review.googlesource.com/c/gerrit/+/582201):
    ...
    Creating an AI review prompt is only useful for authenticated users, so the
    "Create AI Review Prompt" action and dialog are now hidden for anonymous
    users.

  * [Change 595162](https://gerrit-review.googlesource.com/c/gerrit/+/595162):
    Fix syntax highlighting for Ada files.
...
* Enhancements
  ...
  * [Issue 522635656](https://issues.gerritcodereview.com/issues/522635656):
    ...
    Provides administrators and group owners the ability to remove inactive
    users from Gerrit groups. This enhancement helps maintain cleaner access
    control lists, prevents legacy accounts from cluttering group memberships,
    and improves overall access auditing.

  * [Change 588201](https://gerrit-review.googlesource.com/c/gerrit/+/588201):
    Introduce filtering ability for change cleanup.

    Expands the change cleanup functionality by introducing a new query option.
    This gives administrators greater flexibility and precision when defining
    the specific criteria for filtering and cleaning up old abandoned changes.

  * [Change 579121](https://gerrit-review.googlesource.com/c/gerrit/+/579121):
    Optimize plain label queries.

    Label queries without group or count constraints no longer trigger
    post-filtering, avoiding unnecessary NoteDB reads and significantly
    improving query performance.

* Plugins

  * Update replication to `3e623c750`

    You can retrieve the full list of changes with:
    ```
    git log --oneline --no-merges 9b78af806..3e623c750
    ```

    Notable changes are:
    - 57636c9 Extract pack-copy logic into ProjectRepairer
    - fbf435c Repair destinations end-to-end in repair command
    - 19b159d Add explicit project exclusion support
    - 6c7ba53 Add SSH command to repair projects
    - bdf6d45 Support configurable git binary path for SSH remotes
    - e2c7574 Fix `--remote` option to replication start
    - 64ac69d Add RoundRobin URL selection per remote
...
* JGit changes
  ...
  * Bump JGit to c46df0c50f
    ...
    Notable changes are:
    - 8feffbab ReceivePack: Add option to retain PackParser's object IDs
    - eeb86858 ReceivePack: reject creation of one level refs such as refs/main
    - f2c3bdde Document JGit receive command size options
    - 9786d733 Document receive.autogc configuration option
    - 41c2f03b Introduce `fetch.autogc` to disable concurrent GC upon fetches
    - 189b33dc FetchProcess: Report lock failure when destination ref changes
    - 804e29be RefDirectory#refreshPathToLooseRef: also refresh loose ref itself
    - 08212048e TreeWalkConnectivityChecker: Support non-commit objects in refs
    - b7bd2124 `[ssh]` Reject host certificates if the certified key is revoked
...
* Dependency Updates
  ...
  * Embedded `jetty` server updated from `9.4` to `12.1.10`. Jetty 9 has reached
    end-of-life and future Jetty 9 releases will no longer be published to Maven
    Central after January 2026. The upgrade preserves compatibility with
    existing `javax.servlet`-based plugins through Jetty's EE8 adapter modules
    and does not require plugin migration to Jakarta EE.
