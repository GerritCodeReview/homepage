...
* Hide the the bulk action checkbox until all changes are loaded.
...
* Added user kind (`SERVICE_USER` or `HUMAN_USER`) to push and REST API latency metrics
  for allowing to track the performance perceived by interactive users vs. the jobs and
  robots.

* Included callers in the performance summary log, allowing an easier troubleshooting
