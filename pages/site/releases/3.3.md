...
* [Issue 13670](https://bugs.chromium.org/p/gerrit/issues/detail?id=13670);
  Introduce the [cache.openFiles](https://gerrit-documentation.storage.googleapis.com/Documentation/3.3.0/config-gerrit.html#cache.openFiles) setting.

  Allow to specify the number of additional file descriptors to adjust the
  open file ulimit upon gerrit daemon startup.

  Persistent caches might require the allocation of additional file
  descriptors depending on their configuration and backend engine.

  This setting allows to increase the limit of allowed file descriptors
  that can be opened all at once.
