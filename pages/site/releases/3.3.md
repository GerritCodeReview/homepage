...
* The reply dialog posts patchset level comments instead of change messages.
  ...
  "/rerun"). The option to revert this behavior will be removed in
  Gerrit 3.4. Until then, administrators need to have migrated.

* Include patchset level comment as message in comment-added event.

  To workaround the problem that patchset level comments are not
  exposed yet in `comment-added` event, expose patchset level comment
  in `comment-added` event. This is included per default and can be
  disabled with [event.comment-added.publishPatchSetLevelComment](https://gerrit-review.googlesource.com/Documentation/config-gerrit.html#event)
  option.
