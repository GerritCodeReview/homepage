...
* Core issues
  ...
  * [Issue 13800](https://bugs.chromium.org/p/gerrit/issues/detail?id=13800);
    Gerrit stream events do not include review comments body.

    Introduce a new setting
    [`event.comment-added.publishPatchSetLevelComment`](https://gerrit-documentation.storage.googleapis.com/Documentation/3.3.0/config-gerrit.html#event.comment-added.publishpatchsetlevelcomment)
    for preserving the review message in the change's stream events also when
    posted as part of the patchset-level comment.
