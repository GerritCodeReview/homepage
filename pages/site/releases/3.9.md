...
`User suggest Edits` was introduced in `3.7.2` as an experimental feature,
disabled by default. With Gerrit `3.9`, this feature is no longer experimental
and `User Suggest Edits` is enabled by default.
...
The API Changes.id(int) is flagged as deprecated to prevent using it from plugins
and fail to locate changes imported from Gerrit servers with a different server-id.
...
[Change 394445](https://gerrit-review.googlesource.com/c/gerrit/+/394445) that
has pulled 64 unwanted commits from the master branch. As a consequence, the
version has been declared defective and withdrawn.

The upgrade from 3.9.0 needs to follow the instructions of the _"Offline upgrade"_
because it contains an incompatible Lucene Engine that won't be able to read the
legacy index data.
