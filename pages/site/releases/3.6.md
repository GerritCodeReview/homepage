...
- Easier to understand: they are expressed using simple boolean expressions using higher-level change search predicates
  and common boolean operator.
- Lower footprint: they do not require backtracking in their evaluation, consuming less CPU cycles
- Richer GUI: the lacking of requirements can be easily displayed and clearly understood by reviewers and change owners

Existing projects relying on Prolog would still work as expected, allowing a smoother project transition.

Submit requirements also introduce new handy functions that allow to achieve complex rules equivalent to the old Prolog
rules but with an easier syntax:
...
* Support for CentOS is dropped and the base image replaced by AlmaLinux
  ...
  hard for the Gerrit community to continue to support existing Docker setups on CentOS.
  The default base image uses now [AlmaLinux 8](https://almalinux.org/) which promises to have a much longer
  life as open-source OS and has an easy [migration tool from CentOS 8](https://github.com/AlmaLinux/almalinux-deploy)
  available.
...
* Apache Commons Lang v2 is removed
  ...
  use of the older lang v2 from its dependencies. Existing plugins and scripts that may have used Apache Commons Lang v2
  would have to be modified and rebuilt to use the new library.
  Alternatively, they can declare the older Apache Commons Lang v2 as an explicit dependency.
...
Support validation options for branch creation to be used in RefOperationValidationListener implemented in plugins.

* [Change 330200](https://gerrit-review.googlesource.com/330200):
Add support for specifying validation options in rebase and cherry-pick REST endpoint and implemented in plugins.
...
Add syntax highlighting for Markdown in comments
  ...
  highlight.js already has support for Markdown and is reused in the syntax styles in Gerrit comments.
...
Show Change actions regardless if logged in or not, because the gr-change-actions correctly hides the actions
that the current user cannot do.

* [Change 333299](https://gerrit-review.googlesource.com/333299):
Remove the front-end metric `diffViewFullyLoaded` replaced by diffViewDiplayed to represent highlighting a whole file.
...
Fix threading issue in diff cache
  ...
  in computing diffs. The computation failed with a Zlib exception making it look like
  the packfile is currupted, even though fsck reported the repository and the packfile
  as sane.
