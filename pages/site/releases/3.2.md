...
Download: **[3.2.12](https://gerrit-releases.storage.googleapis.com/gerrit-3.2.12.war)**
...
Documentation: **[3.2.12](https://gerrit-documentation.storage.googleapis.com/Documentation/3.2.12/index.html)**
...
A new `Command-Status` field has been added to the `httpd_log` and
the `httpd_log.json`

Whilst the HTTP `statusCode` indicates whether the request was successfully
completed, the `Command-Status` indicates the overall result of the git _
command_ over HTTP.

For instance, this allows to track successful `git-upload-pack`s (`statusCode`
200 OK), whose transfer was interrupted, ultimately causing the command to fail.

It is currently populated only for the transfer phase of `git-upload-pack`
commands, can be found after the `User-Agent` field and has 2 possible values:

- `-1`: The `git-upload-pack` transfer command was ultimately not successful
- `0`: The `git-upload-pack` transfer command was ultimately successful

As for all others fields, if the `Command-Status` field is not present, a "-" is
substituted in the `httpd_log` and omitted in the `httpd_log.json`.
