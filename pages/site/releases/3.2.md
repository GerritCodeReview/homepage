...
[`gerrit.rollingUpgrade`](http://gerrit-documentation.storage.googleapis.com/Documentation/3.2.0/config-gerrit.html)
to `true`.
...
1. Have Gerrit masters upgraded to v3.1.5 in high-availability configuration,
   healthy and able to handle the incoming traffic properly.
2. Set `gerrit.rollingUpgrade` to `true` in `gerrit.config` on both Gerrit masters.
...
9. Remove `gerrit.rollingUpgrade` from `gerrit.config` on both Gerrit masters.
