...
* Bug fixes
  ...
  * [Issue 14930](https://bugs.chromium.org/p/gerrit/issues/detail?id=14930):
    ...
    For instance, this allows to track successful `git-upload-pack`s
    (`statusCode` 200 OK), whose transfer was interrupted, ultimately causing
    the command to fail.

    It is currently populated only for the transfer phase of `git-upload-pack`
    commands, can be found after the `User-Agent` field and has 2 possible
    values:
    ...
    As for all others fields, if the `Command-Status` field is not present, a
    "-" is substituted in the `httpd_log` and omitted in the `httpd_log.json`.
