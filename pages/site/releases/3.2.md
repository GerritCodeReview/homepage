...
Gerrit supports zero-downtime upgrade from Gerrit v3.1.5 when configured using
a [high-availability configuration](https://gerrit.googlesource.com/plugins/high-availability/+/refs/heads/master/README.md), when the Git repositories are stored in a shared filesystem such as
NFS or similar.

For upgrading with zero-downtime, you should enable the rolling upgrade migration
in `gerrit.config` and set the [`gerrit.rollingUpgrade`](http://gerrit-documentation.storage.googleapis.com/Documentation/3.2.0/config-gerrit.html) to `true`.

The zero-downtime upgrade consists of the following steps:

1. Have Gerrit v3.1.5 in high-availability configuration
2. Set `gerrit.rollingUpgrade` to `true` in `gerrit.config`
3. Shutdown the first Gerrit node, upgrade and then startup the node
4. Wait for the first Gerrit node to become healthy
5. Shutdown the second Gerrit node, upgrade and then startup the node
6. Remote `gerrit.rollingUpgrade` from `gerrit.config`

**NOTE**: During the zero-downtime upgrade, the nodes may experience an increase of CPU and memory
utilisation due to the online reindexing activity. If during testing emerge that the nodes
