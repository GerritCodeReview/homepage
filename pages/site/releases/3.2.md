...
* Number of comments to 5000 and their associated size to 16k. The limites can be
customized in `gerrit.config` using the
...
Existing settings `change.api.excludeChangeInMergeable` and `index.change.indexMergeable`
...
The following new additional cache have been introduced and can be customized on
`gerrit.config`:
...
- `gerrit sequence show`: show the current sequences value from All-Projects.git
- `gerrit sequence set`: set the next value for sequences in All-Projects.git
...
the cherry-picked change includes a new field in the commit message footer
...
* Revisit the term `slave` as `replica` in the Gerrit documentation, for alignment to the
