...
* Security Fixes
  ...
  * [Issue 486131256](https://issues.gerritcodereview.com/issues/486131256)
  ...
  Create/update change with submit option bypasses ACLs when used with the `change.submitWholeTopic = true`.
    ...
    A Gerrit user with permissions to create/update changes with the submit option, could submit other changes
    when `change.submitWholeTopic=true` is set on `gerrit.config`. Submit to a Gerrit branch
    can force-merge any visible change by linking it via a shared topic.
