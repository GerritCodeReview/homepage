...
Download: **[3.4.3](https://gerrit-releases.storage.googleapis.com/gerrit-3.4.3.war)**
...
Documentation: **[3.4.3](https://gerrit-documentation.storage.googleapis.com/Documentation/3.4.3/index.html)**
...
3. Revert the index changes
   * Edit the ```index/gerrit_index.config``` to mark ```[index "changes_0060"]``` as
     ```ready = true``` and all other ```changes_*``` indexes as ```ready = false```.
   * Online reindex will then catch up any changes made while the new service was running.
   * Users of the HA plugin can modify the update timestamp to before the upgrade; see
     [documentation](https://gerrit.googlesource.com/plugins/high-availability/+/refs/heads/master/src/main/resources/Documentation/about.md#last-index-update-timestamp-storage)
   * Alternatively you can perform a complete offline reindex, although this may take considerable
     time on large sites.
     ```sh
     java -jar gerrit.war reindex --index changes -d site_path
     ```
4. Startup Gerrit v3.3.x server
