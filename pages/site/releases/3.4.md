...
#### Weak kex algorithms are disabled per default in SSH transport.

As of MINA SSHD 2.6.0, weak kex algorithms are disabled per default in SSH transport.
See [issue](https://issues.apache.org/jira/browse/SSHD-1004) for more details. Prior
to this release, it was recommended to disable weak kex algorithms in gerrit config:
...
As of Gerrit 3.4.0 the weak kex algorithms are disabled per default and cannot be
used, see [Issue 14644](https://crbug.com/gerrit/14644).

The problem is fixed in: 3.4.1. To re-enable weak kex algorithms, the option:
`sshd.enableDeprecatedKexAlgorithms` should be set to `true`.
...
* Breaking changes
  ...
  * Elasticsearch: Support for [EOL version](https://www.elastic.co/support/eol)
    7.4 is discontinued, as it became EOL in early April.

  * [Issue 14644](https://crbug.com/gerrit/14644)
    Weak security settings removed in SSH transport.

    To re-enable weak kex algorithms, the option should be set to `true`:
