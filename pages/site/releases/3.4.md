...
* The introduction of the `auth.userNameCaseInsensitive`-option required a change
  in the APIs used to create `ExternalId`, `ExternalId.Key` and `AuthRequest`-
  objects. These changes may affect plugins using these objects. Perform the
  following adaptations to adapt to the changes:

  * Use `ExternalIdFactory.create*()` and `ExternalIdFactory.parse()` instead
    of `ExternalId.create*()` and `ExternalId.parse()`. The `ExternalIdFactory`
    can be injected via Guice.
  * Use `ExternalIdKeyFactory.create()` and `ExternalIdKeyFactory.parse()` instead
    of `ExternalIdKeyFactory.Key.create()` and `ExternalIdKeyFactory.Key.parse()`.
    The `ExternalIdKeyFactory` can be injected via Guice.
  * Use `AuthRequest.Factory.createFor*()` instead of `AuthRequest.for*()` and
    `AuthRequest.Factory#create(ExternalId)` instead of `AuthRequest(ExternalId)`.
    The `AuthRequest.Factory` can be injected via Guice.

## Native packaging

## New features

* [Optional case insensitive username handling](https://gerrit-review.googlesource.com/Documentation/config-gerrit.html#auth.userNameCaseInsensitive)

  Gerrit originally handled usernames case sensitive. The newly added
  [`auth.userNameCaseInsensitive`-option](https://gerrit-review.googlesource.com/Documentation/config-gerrit.html#auth.userNameCaseInsensitive)
  allows to switch to case insensitive username matching. Switching between
  case sensitive and case insensitive username matching requires the migration
  of external IDs. This migration requires a downtime and cannot be performed online.
  The [`ChangeExternalIdCaseSensitivity`-tool](https://gerrit-review.googlesource.com/Documentation/pgm-ChangeExternalIdCaseSensitivity.html)
  can be used to perform the migration in both directions.
