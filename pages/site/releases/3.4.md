...
Computing `is:mergeable` has a huge runtime computation complexity depending
...
to change anything. Otherwise, the restore the previous behaviour this option:
`change.mergeabilityComputationBehavior` should be changed to `API_REF_UPDATED_AND_CHANGE_REINDEX`.
