...
The new `changes_by_project` cache will make the `git-upload-pack` failing intermittently
on projects containing changes coming from foreign `serverId` that should have been
ignored.

  The introduction of the `changes_by_project` cache is incompatible with projects
  having changes `/meta` refs coming from other Gerrit `serverId` that were previously
  ignored. The cache makes the `git-upload-pack` failing with internal server errors
  intermittently, depending on the status of the cache.
  As workaround, disable the `changes_by_project` cache as follows in `gerrit.config`:
