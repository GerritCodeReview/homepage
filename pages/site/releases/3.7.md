...
* New Features
  ...
  * [Change 406237](https://gerrit-review.googlesource.com/c/gerrit/+/406237):
  Add metrics for `git/upload-pack/bitmap_index_misses_count` and a separate one for missing bitmap indexes.
    ...
    Add the git/upload-pack/bitmap_index_misses_count for begin used
  ...
  * [Change 406004](https://gerrit-review.googlesource.com/c/gerrit/+/406004):
  Add timer for git/upload-pack/phase_searching_for_* phases.
    ...
    but did not cover until
    now, the "Finding sources" phase, which is critical for an optimal
    Git/HTTP performance.

    Add the git/upload-pack/phase_searching_for_* so that the Gerrit
    admin can graph how these are performing and spot any potential
    issues.
  ...
  * [Change 404718](https://gerrit-review.googlesource.com/c/gerrit/+/404718):
  Add timer for `git/upload-pack/phase_negotiating`.
    ...
    when the client uses Git/HTTP, which is counter-intuitive.
    the negotiation phase, which is critical for an optimal Git/HTTP
    performance.

    Add the git/upload-pack/phase_negotiating so that the Gerrit admin
  ...
  * [Change 402497](https://gerrit-review.googlesource.com/c/gerrit/+/402497):
  Introduce configurable shutdown timeout
    ...
    Currently we only wait 30 seconds on top of either httpd or sshd
    gracefulStopTimeout before terminating Gerrit, in some instances this
    might not be enough.
    Make it configurable so end user can decide what's best for their use
    case.

  * [Change 393037](https://gerrit-review.googlesource.com/c/gerrit/+/393037):
  Bazel: Add support for BuildBuddy RBE provider.

    This changes add support for a new RBE provider. BuildBuddy
    provides an open-core suite of enterprise features for Bazel.
    Included are a Remote Build Execution service, a shared build
    artifact cache, and a build result UI. It's available as a
    fully-managed cloud service or as an easy to deploy on-prem
    solution. The service is free to use for individuals and
...
* Performance Fixes
  ...
  * [Change 392634](https://gerrit-review.googlesource.com/c/gerrit/+/392634):
  Improved performance for conflicts: operator and other mergeability checks when submit type is `Merge-If-Necessary`.
    ...
    Optimized canFastForwardOrMerge that de-duplicates the potentially expensive hasMissingDependencies() call
    for callers that needed to do `canFastForward() || canMerge()`.
...
* Security Fixes
  ...
  * [Change 403721](https://gerrit-review.googlesource.com/c/gerrit/+/403721):
  Bump SSHD version to 2.12.0 
    ...
    Includes the upstream fix for [CVE-2023-48795](https://nvd.nist.gov/vuln/detail/CVE-2023-48795) 
    mitigating the [Terrapin attack](https://www.terrapin-attack.com/) in JGit.
...
* Bug Fixes
  ...
  * [Change 404937](https://gerrit-review.googlesource.com/c/gerrit/+/404937):
    Fix copyable change links when base URL has a path.
      ...
      Remove extra base URL prefix in gr-copy-links. links are crafted from
      createChangeUrl which already returns an URL with the base URL. There is
      thus no need to add the base url or prepend the origin.

  * [Change 394818](https://gerrit-review.googlesource.com/c/gerrit/+/394818):
    https://issues.gerritcodereview.com/issues/311925527
  ...
  * [Change 395677](https://gerrit-review.googlesource.com/c/gerrit/+/395677):
    https://issues.gerritcodereview.com/issues/314113030
    Fail the change reindex operation upon StorageException(s).

      Before this change, every time one change indexing throws a
      StorageException, it disappears from the index. every time one change indexing throws a
  ...
  * [Change 402157](https://gerrit-review.googlesource.com/c/gerrit/+/402157):
    https://issues.gerritcodereview.com/issues/318745940
  ...
  * [Change 395741](https://gerrit-review.googlesource.com/c/gerrit/+/395741):
    Fix event parse exception when changing repository HEAD.
      ...
      In order to successfully deserialize events, we need to explicitly
      register a type and class for `ProjectHeadUpdatedEvent` in `EventType`.
      Without this a `JsonParseExecption`: _Uknown event type:
      project-head-updated_ will be thrown.
