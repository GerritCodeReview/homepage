...
available when the plugin is loaded. An example of this is in the replication
plugin, which needs to use the group cache when `authGroup` is used in a
...
is loaded during site startup. Until a fix is available, please use one of
...
command, and then issue a `plugin reload replication` command over ssh. Note
that this requires the `plugins.allowRemoteAdmin` configuration to be
enabled.
