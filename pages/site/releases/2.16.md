...
It is very unusual for Gerrit to require an off-line reindex inside a point release.
...
The reindex of projects, accounts and groups needs to be performed when Gerrit is not active
and after the gerrit.war has been upgraded to the new release.
...
* [Issue 6051](https://bugs.chromium.org/p/gerrit/issues/detail?id=6051):
  PolyGerrit: Update highlight.js to master branch. This fixes some syntax highlighting
  issues with plain C. This also highlights java 10 syntax and adds some new
  files extensions like .properties.

* [Issue 9909](https://bugs.chromium.org/p/gerrit/issues/detail?id=9909):
  PolyGerrit: Do not crash PolyGerrit UI if permitted labels array is empty

* [Issue 10324](https://bugs.chromium.org/p/gerrit/issues/detail?id=10324):
  PolyGerrit: Allow plugins (e.g. delete-project) to control the cache invalidation
  and display new content immediately without refreshing the page.

* Fix broken links in gr-icons.html.


* Includes the following fixes from [Gerrit v2.15.9](2.15.html#2159):
  ...
  * [Issue 10151](https://bugs.chromium.org/p/gerrit/issues/detail?id=10151):
    ...
    PolyGerrit: Fix the "delete" button visibility to allow branch deletion.
  ...
  * [Issue 10326](https://bugs.chromium.org/p/gerrit/issues/detail?id=10326):
    Fix the JVM Heap overload during the execution of the `gerrit ls-projects`,
    the project list REST-API or the list of projects from the Gerrit GWT UI.
