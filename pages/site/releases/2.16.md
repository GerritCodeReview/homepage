...
Download: **[2.16.10](https://gerrit-releases.storage.googleapis.com/gerrit-2.16.10.war)**
...
Documentation: **[2.16.10](https://gerrit-documentation.storage.googleapis.com/Documentation/2.16.10/index.html)**
...
### 2.16.11 (in development)

* [Issue 11083](https://bugs.chromium.org/p/gerrit/issues/detail?id=11083):
Wrong newRev on change-merged when merged by push.

* [Issue 11350]() Upgrade highlight.js to latest master revision, which includes also a fix for Kotlin nested
substitutions.

* Upgrade elasticsearch-rest-client to 7.3.1

* Upgrade JGit to 5.1.10.201908230655-r, which includes improvements on the Git racy-read released in
[5.1.9](https://projects.eclipse.org/projects/technology.jgit/releases/5.1.9) and
[5.1.10](https://projects.eclipse.org/projects/technology.jgit/releases/5.1.10)

* [Issue 11325]() Do not updates change set modified date on ReviewDb when a user delete all its draft changes.
...
8a093ffd8c ListChangeComments: Extend ListChangeDrafts
841d0e326e Upgrade Go Bazel rules to the latest version
e673e0361f Fix anchor tag for settings page
4bc9dc5d0a PrologEnvironment: Reduce "setting reductionLimit" log spam
aa85814fe9 ElasticContainer: Upgrade to 6.8.2 image for V6_8 tests
7fa8f2c5a5 Fix email token routing
89c00ca44e Clarify usage of 'parent' option in list files API
9068e67477 Files: Use Gerrit API to get revision parents
6d6e4ff6c6 Add support for "Link Another Identity" in gr-identities
8c44f92d93 CommitApi: Add method to get commit info
cfc2abdf79 Files: Validate parent option to prevent internal server error
fcfb29da82 Fix and expand documentation of REST API to get revision files
bdad1edf91 Remove default bug tracker from _feedbackUrl
f43acb5837 PG: Add shortcuts for dashboard and watched changes
6604bb35e7 PG: Allow empty label values
b5ecff16e2 Fix gr-identities styles
a2ff0963bf Add an extension point to show a small banner next to the search bar
d2a9ad8ead Fix ls-user-ref and use the correct identity for listing refs
52423b142a ReceiveCommits: Fix comparison to ensure correct log message
2a389469be Set sshd.threads to at least 4.
413eb071d6 ReceiveCommits: Bypass commit count when skip-validation is used
0d1543e2e3 Fix gr-group-audit-log to use tbody
77df787ce5 change-merged: set correct newRev on submit by push
91203c9257 config-project-config: Add missing 'rebase always'
bb40fe23c8 Fix input object of IndexChanges REST endpoint
00e3fe6584 Expose Gerrit's GWT client library in the plugin API
c2e3eb5061 Fix transitive load of GWT_PLUGIN_DEPS for in-tree plugin builds
edc1254177 Bazel: Move PLUGIN_DEPS_NEVERLINK declaration to plugin.bzl
002f0384fa Adapt gerrit.sh script to work on Alpine Linux
8109e14607 Make wording of "Merged" html and text templates consistent
4dde1ff7e9 Online index upgrade: skip submit rules for closed changes
88cb135736 BaseInit: Don't throw if schema cannot be upgraded
89753166f9 Error Prone: Increase severity of MissingFail to ERROR
87f9960f0d Do not encode "," when redirecting /projects/ -> /repos/
