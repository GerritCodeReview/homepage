...
See below an overview of the noteworhy changes.
...
**NOTE**: All references to the `com.google.gerrit.extensions` package are abbreviated with the prefix `c.g.g.e`.

* `c.g.g.e.ChangeInfo/RevisionInfo`: Update copy methods to copy fields that have been added recently

* Support allow_conflicts option in cherry-pick input on `c.g.g.e.api.changes.CherryPickInput`
...
* New API for deleting draft comments of a user in `c.g.g.e.api.accounts.AccountApi`

* New annotation `c.g.g.e.annotations.RemoveAfter` for documenting features in Gerrit that can be deprecated
    
* `c.g.g.e.registration.DynamicSet` returns Iterable for entries instead of Iterator

* Support for deleting change message via REST API in `c.g.g.e.api.changes.ChangeMessageApi`

* New common interface for all REST views on collections, `c.g.g.e.restapi.RestCollectionModifyView`,
...
* Support binding a RestView to delete a missing resource through a brand-new `c.g.g.e.restapi.DeleteViewBinder`. See the [relevant change for more details](https://gerrit-review.googlesource.com/q/If64224d502bbfe578133d1255b42902dd4fbe4fb)

* New Api and REST endpoint to reindex a project on `c.g.g.e.api.projects.ProjectApi`.

* New RestCollection{Create,Delete,Modify}View that can be bound for POST and DELETE requests on collections
...
* Deprecate `c.g.g.e.common.InstallPluginInput`

* Support getting account details in `c.g.g.e.api.accounts.AccountApi`.

* New API and REST endpoint for accessing change messages in `c.g.g.e.api.changes.ChangeApi`.

* Add extension API for emails and support setting an email as preferred in 
  `c.g.g.e.api.account.AccountApi` and `c.g.g.e.api.account.EmailApi`.

* Add API and REST endpoints to get/set default edit preferences in `c.g.g.e.api.config.Server`.

* Allow to filter list by project state in `c.g.g.e.api.projects.Projects`.