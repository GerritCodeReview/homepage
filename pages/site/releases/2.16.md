...
When a older gerrit site is migrated to 2.16, then schema
migration should be first performed to the latest 2.11.x release.

To upgrade to the latest 2.11.x release:

``` sh
  java -jar gerrit-2.11.latest.war init -d ${site_path}
```

* 2.16.16 or earlier only support upgrading from 2.15.x.
* 2.16.17 or newer support upgrading directly from 2.11.x or newer.
* Using the latest 2.16.x version is recommended.
