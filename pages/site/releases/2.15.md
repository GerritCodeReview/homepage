...
Allow to override autodetected Lucene index configuration.
  ...
  The new settings `index.name.maxMergeCount`, `index.name.maxThreadCount` and
  `index.name.enableAutoIOThrottle` allow to manually configure the Lucene index,
  rather than using autodetected values, to improve performance.
