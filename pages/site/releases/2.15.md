...
* Security Fixes
  ...
  * [Issue 13858](https://bugs.chromium.org/p/gerrit/issues/detail?id=13858)
  ...
  Avoid creating HTTP Sessions for every Git/HTTP request.

    The fix for [Issue 9982](https://bugs.chromium.org/p/gerrit/issues/detail?id=9982)
    triggered the creation of a new HTTP Session for every invocation a Git/HTTP
    request.

    The mistake was to track the HTTP session instead of the Gerrit session in
    the audit record. The HTTP Servlet API specs say that any attempt to access
    the current session of an incoming request would result in the creation of
    a brand-new session.

    The session involuntarily created also had an expiry time equal to zero,
    which prevented the session housekeeper to reclaim them later on, as they
    were unused.

    To rectify use the Gerrit session-id, retrieving from the proper web session
    if active and logged in and make sure in tests that no HTTP sessions
    are created as a result of a Git/HTTP request.
