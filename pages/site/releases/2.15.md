...
* [Issue 11363](https://bugs.chromium.org/p/gerrit/issues/detail?id=11363):
Fail early when creating an account with invalid SSH key.

  When an account creation request included an invalid SSH key, the request
  failed but the account was still created (without the SSH key). This
  is fixed so that the request fails before the account is created.
